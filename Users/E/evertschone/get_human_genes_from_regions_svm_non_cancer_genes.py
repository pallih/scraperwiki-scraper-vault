...
for x in range(1,sheet.nrows):    #for each region scrape the ensemble url for genes
    ...
    for tr in root.cssselect("table[class='ss autocenter'] tr"):    #for each gene in the region
        ...
        if len(tds) > 0:
            ...
            if genename not in knowngenelist:    #check if the gene is a possible oncogene from the sanger list of somatic mutations in cancer (COSMIC)
                ...
                if len(p) > 0:
                    ...
                    data = {     #format data
                        ...
                        'mutationratio' : 'n/a'   #mutation ratio from the cosmic database
                    }
                    print data
