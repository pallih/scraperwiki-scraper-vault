...
def getUser(eziobianchi):
    url = 'http://api.twitter.com/1/users/show.json?screen_name=' + eziobianchi
    ...
    #mentions_url = 'http://search.twitter.com/search.json?&rpp=100&ref=' + eziobianchi
    ...
    #inicializa tabela
    void = {'id': 0}
    scraperwiki.sqlite.save(['id'], void, table_name=eziobianchi)
    

def getStatus(eziobianchi):
    options = '&include_entities=true&include_rts=true&count=200&page='
    page = 1
    while page > 0:
        url = 'http://api.twitter.com/1/statuses/user_timeline.json?screen_name=' + eziobianchi + options + str(page)
        full_timeline = urllib.urlopen(url).read()
        full_timeline = json.loads(full_timeline)

        if full_timeline == []:
            print 'Baixou ' + str(page) + ' paginas de ' + eziobianchi
            page = -1
        elif 'error' in full_timeline:
            print 'Erro encontrado: ' + full_timeline['error']
            print 'Na url: ' + url
        elif scraperwiki.sqlite.select('id from %(o)s where id==%(a)s' % {'o': eziobianchi, 'a': full_timeline[len(full_timeline)-1]['id']}):
            print 'Tweets dessa pagina ' + eziobianchi + ' atualizados!'
            pass
        else:
            for status in full_timeline:
                ...
                scraperwiki.sqlite.save(['id'], timeline, table_name=eziobianchi)
        page += 1

eziobianchi_done = [u'eziobianchi']
eziobianchi = [u'eziobianchi']


for eziobianchi in eziobianchi:
    screenname = eziobianchi
    getUser(eziobianchi)
    getStatus(eziobianchi)import urllib
import json
import scraperwiki
