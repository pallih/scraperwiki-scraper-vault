...
#Apparently not a good Python way of doing things, but whatever...
    for i in range(0,len(tbindex)) :
        ...
        dates = date.encode('raw_unicode_escape').split("\u2192")

        if len(dates)==2:
            datebegin= parser.parse(dates[0])
            dateend = parser.parse(dates[1])
        elif len(dates)==1:
            datebegin= parser.parse(dates[0])
            dateend = datebegin
        else:
        ...
        scraperwiki.sqlite.save(unique_keys=['name'], 
                            data={'name':name,'url':url,'location':location,'datebegin':str(datebegin.date()),'dateend':str(dateend.date())})
