...
def get_company_info(url):
    ...
    try:
        ...
        flag=0
        for debtratio in tree2.findall('.//tr//td'):
            if(flag<5 and flag>0):        
                if(flag==1):
                    keydebtratio=debtratio.text + ','
                elif(flag==4):
                    keydebtratio= keydebtratio + debtratio.text
                else:
                    keydebtratio= keydebtratio + debtratio.text + ','
                flag=flag+1
            if(flag>4):
                break
            if(debtratio.text=="Debt-Equity Ratio"):
                flag=flag+1
        
        flag=0
        for peratio in tree2.findall('.//tr//td'):
            if(flag<5 and flag>0):        
                if(flag==1):
                    keyperatio=peratio.text + ','
                elif(flag==4):
                    keyperatio= keyperatio + peratio.text + ','
                else:
                    keyperatio= keyperatio + peratio.text + ','
                flag=flag+1
            if(flag>4):
        ...
        flag=0
        for eps in tree2.findall('.//tr//td'):
            if(flag<5 and flag>0):        
                if(flag==1):
                    keyepsratio=eps.text + ','
                elif(flag==4):
                    keyepsratio= keyepsratio + eps.text
                else:
                    keyepsratio= keyepsratio + eps.text + ','
                flag=flag+1
            if(flag>4):
