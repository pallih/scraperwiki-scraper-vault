...
def process_google_spreadsheet(url, post_url):
    """
    ...
    spreadsheet_url = "http://spreadsheets.google.com/ccc?key=" + key
...
def docs_links_from_html(url, root):
    ...
    for link in links:
        ...
        if link_url.find("spreadsheets.google.com") >= 0:
            key = process_google_spreadsheet(link_url, post_url)
            if key == None:
                continue
...
def reprocess_all_spreadsheets():
    ...
    for row in data:
        process_google_spreadsheet(row["url"], row["posturl"])
