...
scraperwiki::sqliteexecute("CREATE TABLE IF NOT EXISTS turk_table (ID int,COTTAGE_URL string , TEXT string, DATE string,
dateFrom  TEXT string, dateTo  TEXT string, duration  TEXT string, originalPrice  TEXT string, discountedPrice  TEXT string,  percentage TEXT string,  discountAmount TEXT string)");
 $id = 0;
...
####################################################################################################################
 ...
 scraperwiki::attach("special_offers_coast_and_country") ;
 # get an array of the cottage data to scrape
 $cottData = scraperwiki::select("COTTAGE_URL, PRICE_HIGH, PRICE_LOW from 'special_offers_coast_and_country'.SWDATA order by COTTAGE_URL");
...
/*  Get the Data  */
    ...
    /*  save the data  */
    if(!$OK){
    scraperwiki::sqliteexecute("insert into turk_table values (?,?,?,?,?,?,?,?,?,?,?)",array($i,trim($url.$value['COTTAGE_URL']), $specialOffer , $date->getTimestamp(),$dateFrom,$dateTo, $duration, $originalPrice, $discountedPrice,  $percentage,  $discountAmount));
        $i++;
    }else{
...
$dateTo =   date_create_from_format('jS F Y',$dateTo);
       $record = array(
          ...
          'PRICE_HIGH'        => trim($cottData[0]['PRICE_HIGH']),
          'PRICE_LOW'         => trim($cottData[0]['PRICE_LOW']),
          'CHANGEOVERDAY'     => trim($cottData[0]['CHANGEOVERDAY']),
          'COTTAGE_NAME'      =>  trim($cottData[0]['COTTAGE_NAME']),
          'SLEEPS'            =>  trim($cottData[0]['SLEEPS']),
          'BEDROOMS'          =>  trim($cottData[0]['BEDROOMS']),
          'BATHROOMS'         =>  trim($cottData[0]['BATHROOMS']),
          'PETS'              =>  trim($cottData[0]['PETS']),
          'COTTAGE_REFERENCE' =>  trim($cottData[0]['COTTAGE_REFERENCE']),
          'FEATURES'          =>  trim($cottData[0]['FEATURES']),  
          'Summary'           =>  trim($cottData[0]['Summary']),
          'DESCRIPTION'       =>  trim($cottData[0]['DESCRIPTION']),
          'Location'          =>  trim($cottData[0]['Location']),
          'LONGITUDE'         =>  trim($cottData[0]['LONGITUDE']),
          'LATITUDE'          =>  trim($cottData[0]['LATITUDE']),
          'IMAGES'            =>  trim($cottData[0]['IMAGES']),
          'Agent'             => "coastandcountry.co.uk",

          'DATE_FROM'         => $dateFrom->format('Y-m-d'),
          'DATE_TO'           => $dateTo->format('Y-m-d'),

          #'DATE_FROM'         => date_create($dateFrom),
          #'DATE_TO'           => date_create($dateTo),
          'DURATION'          => $duration,
      ...
      scraperwiki::sqliteexecute("insert into turk_table values (?,?,?,?,?,?,?,?,?,?,?)",array($i,trim($url.$value['COTTAGE_URL']), $specialOffer , $date->getTimestamp(),$dateFrom,$dateTo, $duration, $originalPrice, $discountedPrice,  $percentage,  $discountAmount,$value['COTTAGE_URL']));
     $i++;
}
