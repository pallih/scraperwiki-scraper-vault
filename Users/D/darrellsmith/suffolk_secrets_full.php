...
function fourOhFour($html){
 ...
 scraperwiki::attach("suffolk_secrets_summary") ;
 $cottData = scraperwiki::select(" COTTAGE_ID, COTTAGE_URL, SLEEPS, BEDROOMS, PETS,COTTAGE_NAME, PRICE_LOW from 'suffolk_secrets_summary'.SWDATA order by COTTAGE_URL");
...
// changeover
  foreach($dom->find('div[class=property_container]') as $box){
    foreach($box->find('div[class=property_rightcol]') as $data){
...
switch ($changeoverKey) {
  ...
  // features 
  $cottage_facil = "";
  foreach($dom->find('div[class=property_leftcol]')as $data){
    foreach($data->find('div[id=property-description]')as $item){
      foreach($item->find('div[class=inner-padding]')as $description){
        $descArray = explode("<p>",$description);
                 
        $cott_desc = strip_tags($descArray[1]);
        $area_desc = strip_tags($descArray[3]);
        $features = strip_tags($descArray[2]);
  ...
  $pic = "";
  foreach($dom->find('div[id=carousel-container]')as $item){
 ...
 // long lat
 $coords = substr($html,strpos($html,"#property-map-container",10000),100); 
 print_r($coords);
