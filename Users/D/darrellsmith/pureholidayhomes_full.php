...
foreach($cottData as $value){
  ...
  $priceArray = array();
  if($found == 0){
    foreach($dom->find('table[class=phh_page_property_DataTable]')as $item){
      $found = 1;
      foreach($item->find('tr')as $data){
        $pricesArr = explode("<td>",$data);
        if(count($pricesArr) > 3){
            array_push( $priceArray,str_replace("&#163;","",str_replace("</td>","",$pricesArr[3])));
        }
      }
    }
  }
