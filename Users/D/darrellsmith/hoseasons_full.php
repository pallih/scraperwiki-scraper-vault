...
function fourOhFour($html){
 ...
 foreach($dom->find('div[class=cottageDetailsLeft]')as $data){
  ...
  $pics = "";
  for($i=0;$i<=$count;$i++){
    $cottageID = substr($cottage_url,strpos($cottage_url,"-")+1);

  $urlSegs = explode("-",$cottageID);
    

    $cotturl = end($urlSegs);
     

    $picURL = "http://www.hoseasons.co.uk/WebPages/THGCottage/Popup/SupersizeGallery.aspx?ACODE=".$cotturl ."&nextLinkId=ctl00_ContentPlaceHolder_accommodationGallery_nextLink&prevLinkId=ctl00_ContentPlaceHolder_accommodationGallery_prevLink&index=".$i;    
