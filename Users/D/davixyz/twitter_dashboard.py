...
usernames = [
             "Bibliotecacejfe","bibliotequescat","D_Roch",
             "compartim","camptarragona","cultura_cat","ensenyamentcat","ocupaciocat",    
             "territoricat","emergenciescat","FeinaActiva","ticsalut",
             "gencat","iniciacat","icscat","icdones","idescat","llenguacatalana","mediambientcat",
             "memorialcat","meteocat","patrimonigencat","institucionscat",
             "ruralcat","sexejovescat","catsostenible"
            ]
...
for x in usernames:
    ...
    if result["query"] and result["query"]["results"] and result["query"]["results"]["span"]:
        ...
        scraperwiki.datastore.save(unique_keys=["username"],data={"username":x, "tweets":int(replaces(dades[0])),"following":int(replaces(dades[1])), "followers":int(replaces(dades[2])),"listed":int(replaces(dades[3]))}) 
