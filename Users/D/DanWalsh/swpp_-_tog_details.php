...
$html = scraperwiki::scrape("http://swpp.co.uk/members/");
$html = oneline($html);
preg_match_all('|Find .*? in [a-zA-Z\'\- ]+" href=".*?.htm">(.*?)</a>|',$html,$nation);


foreach ($nation[1] as $key=>$val) {
$val = urlencode($val);
$html = scraperwiki::scrape("http://swpp.co.uk/members/".$val.".htm");
$html = oneline($html);
preg_match_all('|<li><a href="([a-zA-Z0-9]+.htm)">|',$html,$link);


foreach ($link[1] as $key=>$val) {
$html = scraperwiki::scrape("http://swpp.co.uk/members/".$val);
$html = oneline($html);
...
scraperwiki::save(array('business'), array(
    'link' => "http://swpp.co.uk/members/".$val,
