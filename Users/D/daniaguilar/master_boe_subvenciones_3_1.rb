...
disposiciones.each do |disp|
  ...
  if (page)
    ...
    disp_text =~ /entidades\s+beneficiarias:(.*)Lo\s+que\s+se\s+hace/ #patron para corta el texto que importa
    if $1.nil? 
      log(disp['id'],disp['url'],"Could not parse data on the document")
      next
    end
#a partir de que se tiene el texto del que se quiere extraer los datos, con expresiones regulares se van desmenuzando los datos
    disp_data = $1
