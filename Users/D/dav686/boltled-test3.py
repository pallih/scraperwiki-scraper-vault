...
class boltSpider(BaseSpider):
    name = "bolt"
    allowed_domains = ["boltled.com"]
    start_urls = [
        "http://boltled.com/store/commercial-lighting.html/",
        "http://boltled.com/store/skin-therapy.html/"
    ]

    def parse(self, response):
        hxs = HtmlXPathSelector(response)
        sites = hxs.select("//ul[@class='products-grid']/li")
        items = []
        for site in sites:
            loader = WebsiteLoader(selector=site)
            loader.add_xpath('name', 'h2/a/text()')
            loader.add_xpath('url', 'a/@href')
            loader.add_xpath('price', 'div[@class="price-box"]//span[@class="price"]/text()')
...
def main():
    ...
    run_spider(boltSpider(), settings)
