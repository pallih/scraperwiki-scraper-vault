...
xls = zfp.open('table-9-local-authority-quinary-2010.xls').read()
...
for rowno in range(male.nrows):
    ...
    data['areacode']=row[0]
    data['areaname']=row[1]+row[2]+row[3] # TODO: trim numbers at end? 
    #data['arealevel']=                   # TODO: count indentation levels?
    data['gender']='male'
    
    for i in range(0,21): # CHECK RANGE!
        tdata=dict(data)
        col=(i*2)+5
        if i<3:
            tdata['age']=('all',0,1)[i]
