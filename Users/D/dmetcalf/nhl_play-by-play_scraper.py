    ...
    for td in tds: # iterate over each of the <td> elements (i.e. elements of the list called "tds")
        if (td.text == "GOAL") and (tds[i - 3].text != "5"): # if the <td>'s contents are "Goal" and the period isn't the 5th (i.e. the shootout)
            if tds[i + 1].text.startswith(hometeamshort): # check if the next table cell starts with the same letters as the home team
                ...
                homecorsi += 1
                #try:
                #    print "Away on ice:", tds[i + 5].text, tds[i + 9].text, tds[i + 13].text, tds[i + 17].text, tds[i + 21].text, tds[i + 25].text
                #    print "Home on ice:", tds[i + 29].text, tds[i + 33].text, tds[i + 37].text, tds[i + 41].text, tds[i + 45].text, tds[i + 49].text
                #except:
                #    pass
                skaters = []
                homeskaters = awayskaters = 0
                hometeamstartsat = 0
                #skaterstext = []
                for n in range(5, 5 + (7 * 4) + 1):
                    #skaters.append(tds[i + n])
                    if len(tds[i + n].cssselect(".bborder")) == 1:
                        hometeamstartsat = i + n
                        break
                    skater = tds[i + n].text
                    if skater == "C" or skater == "L" or skater == "R" or skater == "D":
                        awayskaters += 1
                for n in range(3, 3 + (6 * 4) + 1, 4):
                    skater = tds[hometeamstartsat + n].text
                    if skater == "C" or skater == "L" or skater == "R" or skater == "D":
                        homeskaters += 1
                    elif skater == "G":
                        break
                #print "Away skaters:", awayskaters, "Home skaters:", homeskaters
                    #skaterstext.append(tds[i + n].text)
                #print skaterstext
                #for skater in skaters:
                #    try:
                #        if len(skater.cssselect(".bborder")) == 1:
                #            print "Found bborder!"
                #    except:
                #        pass
                ...
                if tds[i - 2].text == "EV":            
                    awayevcorsi += 1
                #try:
                    #print "i + 5 text is", tds[i + 5].text
                #except:
                    #pass
            ...
            if tds[i + 1].text.startswith(hometeamshort):
                ...
                if tds[i - 2].text == "EV":
                    ...
                    if homegoals == awaygoals:
                        ...
                        leafsfenup2 += 1
        elif td.text == "PEND" and tds[i - 3].text == "4": # if the period ended and it's the 4th (i.e. OT)
            #print tds[i - 1].text
            minutes_to_add = tds[i - 1].text[0:1]
            seconds_to_add = tds[i - 1].text[2:4]
            #print "minutes to add:", minutes_to_add, "seconds to add:", seconds_to_add
            seconds_played += (60 * int(minutes_to_add)) + int(seconds_to_add)
        i += 1
