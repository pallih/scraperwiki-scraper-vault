...
totdata = scraperwiki.sqlite.execute("SELECT SUBSTR(Date,0,8) AS Month, sum(Amount) FROM src.Refined WHERE Date >= ? AND Date < ? GROUP BY Month ORDER BY Month", (start, end)).get("data")
