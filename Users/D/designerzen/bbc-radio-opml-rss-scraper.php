...
$html = scraperwiki::scrape("http://www.bbc.co.uk/radio/opml/bbc_podcast_opml.xml");
...
foreach($dom->find('outline') as $data)
{
    ...
    $rssPageLink     = $data->page;
    # get the name of the artist or some kind of ID
    //print $rssUrl . " - " . $rssDescription;

    # and now save in the database if the URL is valid :)
    $results = array('title'=>$rssTitle, 'feed' => $rssUrl, 'description' => $rssDescription, 'artwork' => $rssImage, 'link'=>$rssPageLink );
