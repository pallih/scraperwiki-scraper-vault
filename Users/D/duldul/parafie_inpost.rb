...
ScraperWiki::attach("sklepsfdpl_scraper_1", "par")
#p ScraperWiki::select("* from par.swdata WHERE `City` = 'Andrychów'")

inpost = Nokogiri::HTML(open('http://www.duldul.pl/inpost.php'), nil, 'utf-8')

inpost.css('li').collect do |row|
  woj = row.inner_text
  
  ids = ScraperWiki::select("ID from par.swdata WHERE `City` LIKE '"+woj+"'")
  p woj if ids == []
end
#p inpost
