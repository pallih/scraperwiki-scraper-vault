...
def shallow_scrape():
    ...
    resultspage = br.open("http://www.edubase.gov.uk/public/quickSearchResult.xhtml")

    moreWorkToDo = True
    c = 1

    while moreWorkToDo and (c<3000):
...
def page_scrape(name, urn):
    print name
    url = "http://www.edubase.gov.uk/establishment/"+name+".xhtml"+"?urn="+urn
    parser = HTMLParser(tree=treebuilders.getTreeBuilder("lxml"))
    page = parser.parse(scrape(url))
