...
SELECT DISTINCT ?hedge_funds
WHERE {
?s foaf:page ?hedge_funds.
?s rdf:type <http://dbpedia.org/ontology/hedge_funds> .
?s dbpedia-owl:country ?country;
   dbpedia2:country ?propcountry .
FILTER (?propcountry = "United States" || ?propcountry = "US")
}'''
US_URL = 'http://dbpedia.org/resource/Category:hedge_funds_in_the_United_States_by_state'
...
def parse_state_data(row):
    if row['predicate'] == 'http://purl.org/dc/terms/subject':
        parse_hedge_fund_data(row['subject'])
...
def parse_hedge_funds_data(resource_url):
    hedge_funds = DBpediaRDF(resource_url)
    data = {
        'resource': resource_url,
        'name': hedge_funds.xpath_text('foaf:name'),
        'organization': hedge_funds.xpath_text('dbpprop:affiliation'),
        'latitude': hedge_funds.xpath_text('geo:lat'),
        'longitude': hedge_funds.xpath_text('geo:long'),
        'website': hedge_funds.xpath_text('dbpprop:website'),
        'description': hedge_funds.xpath_text('dbpedia-owl:abstract'),
        'category': hedge_funds.xpath_text('dbpprop:healthcare'),
    }
