...
def scrape_table(page_url):
    ...
    for x in root.cssselect('.desc'):
        ...
        print "DESC %s" % check_data(x.cssselect('.desc'))
        print "ITEM_LINK %s" % check_data(x.cssselect('.item_link'))
        print "LIST_PRICE %s" % check_data(x.cssselect('.list_price'))
        # print "ADR %s" % check_data(x.xpath("//*[re:test(., '^abc$', 'i')]")

#/h:body/h:div/h:p (Selects <p> tags that are children of <div> tags that are children of the <body> tag)
 
        print "CIT %s" % check_data(x.xpath("*"))
