...
def scrape_people(role='alumni' ):

    # We can scrape different types of people!
    roles = ['student', 'coworker', 'collaborator', 'alumni']
        ...
        if r.status_code==200:
            ...
            if len(dom.cssselect('div.community_list')):
                ...
                for td in dom.cssselect('div.community_list td'):
                    ...
                    if role != 'coworker' and role != 'collaborator':
                        course = get_element_or_none(td, '.subtitle')
                        year = get_element_or_none(td, '.subtitle')
                        if course:
                            person['course'] = course[:-5]
                        if year:
                            try:
                                person['year'] = int(year[:-4])
                            except ValueError:
                                pass
...
def geocode_nationalities(role='alumni'):
    import json
...
scrape_people('alumni' )
geocode_nationalities('alumni' )

scrape_people('student' )
geocode_nationalities('student' )

scrape_people('collaborator' )
geocode_nationalities('collaborator' )

scrape_people('coworker' )
geocode_nationalities('coworker' )import scraperwiki
import requests
import lxml.html
...
scrape_people('coworker' )
geocode_nationalities('coworker' )