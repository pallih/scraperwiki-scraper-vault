...
//retrieve data from cz_regions_ufis_downloader
// form 50 (=40) only!
...
//scraperwiki::attach("cz_regions_ufis_basics", "src0");
//$forms = scraperwiki::select("* from src0.form where form=50 and region IS NULL order by dri ASC,form DESC,period ASC,org_id ASC");
scraperwiki::attach("cz_regions_ufis_downloader", "src");



$forms = scraperwiki::select("* from src.swdata where form='50' ORDER BY region_code, period");

foreach($forms as $k => $form) {
  if ($k < $i) continue;

  $xml_str = $form['xml'];
  $report_number = $form['form'];
  $org_id = $form['org_id']+1-1;
  $year = $form['period'];
