...
//http:http://www.volby.cz/pls/prez2013/pe3?xjazyk=CZ

require 'scraperwiki/simple_html_dom.php';

$url = 'http://www.volby.cz/pls/prez2013/pe3?xjazyk=CZ';                                               //********************************
...
function town($link,$region_code,$region) {
  ...
  $url = 'http://www.volby.cz/pls/prez2013/' . $link;                                               //********************************
  $html = str_replace('&nbsp;','',iconv('ISO-8859-2','UTF-8//TRANSLIT',scraperwiki::scrape($url)));
  $dom = new simple_html_dom();
  $dom->load($html);
    ...
    foreach ($trs as $tr) {
      ...
      if ($tds[0]->plaintext != '-') {
          $data = array(
            ...
            //correct 'zahranici'
            if ($link == 'pe36?xjazyk=CZ') {
                $c = count($tds);
                $data = array(
                    'town' => $tds[$c-2]->plaintext,
                    'town_code' => 1000000 + $tds[$c-1]->plaintext,
                    'link' => html_entity_decode($tds[$c-1]->find('a',0)->href),
                    'region_code' => $region_code,
                    'region' => $region,
                );
            }
