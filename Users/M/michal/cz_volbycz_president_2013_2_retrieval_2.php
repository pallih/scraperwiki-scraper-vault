...
scraperwiki::attach("cz_volbycz_president_2013_2_downloader_2", "src");
...
//echo $trsc;
  ...
  $tds = $tr->find('td');
  $info = array(
    'okrsky' => (($trsc == 3) ? 1 : str_replace(' ','',$tds[0]->plaintext)),
    'okrsky_done_perc' => (($trsc == 3) ? ((str_replace(' ','',$tds[1]->plaintext) > 0) ? '100.00' : 0) : str_replace(',','.',$tds[2]->plaintext)),
  ...
  foreach ($tables as $table) {
    ...
    foreach ($trs as $tr) {
      ...
      if (($tds[0]->plaintext != '')and($tds[0]->plaintext != '-')) {
          ...
          $item['votes'] = str_replace(' ','',$tds[6]->plaintext);
          $item['votes_percentage'] = str_replace(' ','',str_replace(',','.',$tds[5]->plaintext));
          if (($item['candidate_number'] == '6') or ($item['candidate_number'] == '9'))
            $data[] = $item;
      }
    }
  }
