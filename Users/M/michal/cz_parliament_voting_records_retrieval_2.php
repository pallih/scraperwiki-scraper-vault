...
$last_url = scraperwiki::save_var('last_id','');
//get last id
$last_url = scraperwiki::get_var('last_id','');
echo $last_url;

//read the saved tables
scraperwiki::attach("cz_parliament_voting_records_downloader_2", "src");
$rows = scraperwiki::select("url from src.swdata where url>'{$last_url}' order by url");
...
if (!empty($rows)) {
  foreach ($rows as $html) {
    ...
    $html2 = scraperwiki::select("* from src.swdata where url='{$html['url']}'");
    $dom->load(str_replace("&nbsp;"," ",$html2[0]['html']));
    //info
    $data = array('url'=>$html['url']);
    $h2s = $dom->find("h2");
    ...
    $data['needed'] = $needed;
//print_r($data);die();
    scraperwiki::save_sqlite(array('url'),$data,'info');
...
//print_r($data);die();
    scraperwiki::save_sqlite(array('division_url','mp_url','club'),$data,'vote');
