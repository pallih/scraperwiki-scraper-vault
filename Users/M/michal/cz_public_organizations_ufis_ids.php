...
$periods = array('0' => array('value' => '12/2012'));  //temp!!
      ...
      switch ($dri['label']) {
        case 'Místně řízené organizace':
          foreach((array) $regions as  $rkey => $region) {
            ...
            if ($rkey < $r) continue;
              $url = "http://wwwinfo.mfcr.cz/cgi-bin/ufis/iufisorg/organizace.pl?dri={$dri['value']}&obdobi={$period_encoded}&prijmy=0&vydaje=0&pevne=0&vykaz={$form['value']}&vykaznam={$form['value']}&zko={$region['value']}&zkonam={$region['value']}";
              $data = retrieve($url);
        ...
        default:
          foreach ((array) $chapters as $ckey=>$chapter) {
            ...
            if ($ckey < $c) continue;
              $url = "http://wwwinfo.mfcr.cz/cgi-bin/ufis/iufisorg/organizace.pl?dri={$dri['value']}&obdobi={$period_encoded}&prijmy=0&vydaje=0&pevne=0&vykaz={$form['value']}&vykaznam={$form['value']}&kapitola={$chapter['value']}&kapitolanam={$chapter['value']}";
              $data = retrieve($url);
