...
def my_echo(string):
        ...
        if (empty(string)):
                if (dev == 1):
                        ...
                        echo message;
                        echo "\n";
def handle_products(product_link):
        global base_url_host, base_url_scheme, total;
        if (isEmpty(empty)product_link)):
                # There are times when Thule will list discontinued products aspart of their catalog - test for this...
                ...
                if (found == False):
                        ...
                        link_path = explode("/", link_elements.get('path'));
                        ...
                        par_name = (isEmpty(empty)par_name_raw)) ? strip_tags(par_name_raw) : "";
                        # Get the parts image.
                        a_link = html_3->find("a[@id='phcontent_0_ctl00_lbBomImage']");
                        img_raw = (isEmpty(empty)a_link)) ? a_link[0]->find("img",0)->src : "";
