...
def fetch_statuses(max_id, since_id, username, list):
    base_url = 'https://api.twitter.com/1.1/%s/lists/%s/statuses.json?include_entities=false&include_rts=true&per_page=%s' \
...
def fetch_user_timeline(max_id, since_id, username):
    base_url = 'https://api.twitter.com/1.1/statuses/user_timeline.json?include_entities=false&include_rts=true&screen_name=%s&count=%s' \
