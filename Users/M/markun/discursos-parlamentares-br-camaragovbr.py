...
dtInicio = ''
dtFim = ''
orador = '&txOrador=' + 'ULYSSES+GUIMARAES'
CurrentPage = '&CurrentPage='
PageSize = '&PageSize=' + '1000'
options = 'txIndexacao=&BasePesq=plenario&txPartido=&txUF=&txSessao=&listaTipoSessao=&listaTipoInterv=&inFalaPres=&listaTipoFala=&listaFaseSessao=&txAparteante=&listaEtapa=&CampoOrdenacao=dtSessao&TipoOrdenacao=DESC&txTexto=&txSumario='
page_number = '1'
...
def scrape_num():
    first_page = 1
    url = base_url + options + orador + dtInicio + dtFim + PageSize + CurrentPage + str(first_page)
    html = unicode(scraperwiki.scrape(url), 'utf-8', 'ignore')
    soup = BeautifulSoup(html, fromEncoding='utf-8')  
...
for page in range(1, paginas):
    ...
    url = base_url + options + orador + dtInicio + dtFim + PageSize + CurrentPage + str(page)
