...
def getComissions(senator_id, pages):
    ...
    for row in rows:
        ...
        scraperwiki.sqlite.save(["id"], comission, table_name='comissions')
        id += 1
...
def comissionsrun():
    senadores_id = scraperwiki.sqlite.select("id from senadores_list")
    for senador in senadores_id:
       getComissions(senador['id'], 1)

comissionsrun()# Blank Python
import scraperwiki
import urllib
import BeautifulSoup
import re
