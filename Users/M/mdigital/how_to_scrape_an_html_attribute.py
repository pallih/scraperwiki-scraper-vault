...
import scraperwiki
html = scraperwiki.scrape('http://www.ag.senate.gov/about')
...
spans = root.cssselect('span') # get all the <td> tags
for span in spans:
    print lxml.html.tostring(span) # the full HTML tag
    print span.text                # just the text inside the HTML tag
...
for span in spans:
     record = { "span" : span.text } # column name and value
     scraperwiki.sqlite.save(["span"], record) # save the records one by one
