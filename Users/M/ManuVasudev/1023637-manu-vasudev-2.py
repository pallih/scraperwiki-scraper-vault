...
def scrape_table(soup):
    ...
    for row in rows:
        ...
        table_city = row.findAll("td")
        #The number of cells in each row should be five:
        if len(table_city) == 5:
            record['Rank 2009'] = table_city[0].text
            record['Rank 2008'] = table_city[1].text
            record['Country'] = \
            table_city[2].text
            record['City'] = table_city[3].text
            record['Index 2009'] = table_city[4].text
            print record,
            print "-------------"
