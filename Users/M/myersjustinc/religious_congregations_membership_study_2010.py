...
USING_SCRAPERWIKI = False
try:
    import scraperwiki
    USING_SCRAPERWIKI = True
except ImportError:
    pass

# Thanks: https://gist.github.com/1233434
COUNTY_FIPS = {
...
# Thanks: http://stackoverflow.com/a/1733105/803749
...
def parse_page(page_content):
    bodies = defaultdict(county_detail)
    traditions = defaultdict(county_detail)
    families = defaultdict(county_detail)
    
    root = lxml.html.fromstring(page_content)
    table = root.cssselect("table.data")
    if table:
        table = table[0]
    else:
        raise DataNotFound
    ...
    for row in table.cssselect("tr"):
        ...
        body, tradition, family, congregations, adherents, adherence_rate = [
            cell.text_content().strip() for cell in cells
        ]
        if family not in ["Family", "Totals:"]:
            try:
                congregations = int(congregations)
            except ValueError:
                congregations = 0
            
            try:
                adherents = int(adherents.replace(',', ''))
            except ValueError:
                adherents = 0
            
            try:
                adherence_rate = PrettyFloat(adherence_rate)
            except ValueError:
                adherence_rate = 0
            
            bodies[body]["adherence_rate"] += adherence_rate
            bodies[body]["adherents"] += adherents
            bodies[body]["congregations"] += congregations
            
            traditions[tradition]["adherence_rate"] += adherence_rate
            traditions[tradition]["adherents"] += adherents
            traditions[tradition]["congregations"] += congregations
            
            families[family]["adherence_rate"] += adherence_rate
            families[family]["adherents"] += adherents
            families[family]["congregations"] += congregations
...
def main():
    all_counties = {}
    for county_fips in sorted(COUNTY_FIPS.keys()):
        state_fips = county_fips[:2]
        
        url = ''.join([
            'http://www.thearda.com/rcms2010/r/c/', state_fips,
            '/rcms2010_', county_fips, '_county_name_2010.asp'
        ])
        r = requests.get(url)
        
        try:
            if USING_SCRAPERWIKI:
                county_results = parse_page(r.text)
                for body in county_results["bodies"]:
                    scraperwiki.sqlite.save(('fips', 'name',), {
                        "fips": county_fips,
                        "name": body,
                        "congregations": county_results["bodies"][body]["congregations"],
                        "adherents": county_results["bodies"][body]["adherents"],
                        "adherence_rate": county_results["bodies"][body]["adherence_rate"],
                    }, table_name="bodies")
                for tradition in county_results["traditions"]:
                    scraperwiki.sqlite.save(('fips', 'name',), {
                        "fips": county_fips,
                        "name": tradition,
                        "congregations": county_results["traditions"][tradition]["congregations"],
                        "adherents": county_results["traditions"][tradition]["adherents"],
                        "adherence_rate": county_results["traditions"][tradition]["adherence_rate"],
                    }, table_name="traditions")
                for family in county_results["families"]:
                    scraperwiki.sqlite.save(('fips', 'name',), {
                        "fips": county_fips,
                        "name": family,
                        "congregations": county_results["families"][family]["congregations"],
                        "adherents": county_results["families"][family]["adherents"],
                        "adherence_rate": county_results["families"][family]["adherence_rate"],
                    }, table_name="families")
            else:
                all_counties[county_fips] = parse_page(r.text)
        except Exception:
            sys.stderr.write("Had a problem with %s, %s (FIPS %s)\n" % (
                COUNTY_FIPS[county_fips][1],
                COUNTY_FIPS[county_fips][0],
                county_fips
            ))
    
    if not USING_SCRAPERWIKI:
        output_file = open("scraped.json", 'w')
        json.dump(all_counties, output_file)
        output_file.close()

if USING_SCRAPERWIKI or __name__ == '__main__':
    main()
