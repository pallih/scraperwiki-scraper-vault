            ...
            for k in range(1, (no_of_wards+1)):
                ...
                for region in regions:
                        try:
                            ...
                            if(region['value']!=''):
                                data = {'count_id':count, 'vdc_name' : vdc.getText(), 'vdc_id' : vdc['value'], 'district_id':i, 'region_name':region.getText(), 'region_id' : region['value'], 'wards':k, 'vdc_type':vtype }
                                scraperwiki.sqlite.save(unique_keys=['count_id'], data=data)
                                count += 1
                        except:
                            pass
