...
def addArea(areaCode):
    ...
    print datestr 
    url ='http://www.env-it.de/luftdaten/statedata.csv?comp=PM1&version=v1&type=1TMW&date=' + datestr + '&time=900&state=' + areaCode
    print url
...
i = 0
for row in reader:           
    if row['parameter'] == 'Particulate matter - PM10, first measurement':
...
for row in reader:           
    if towerIDList.count(row['station_code']) > 0 and row['station_end_date'] == '':
        ...
        data = {
            'id' : row['station_code'],
            'name' : unicodeName  ,
            'pm10' : areas[areacode][row['station_code']],
