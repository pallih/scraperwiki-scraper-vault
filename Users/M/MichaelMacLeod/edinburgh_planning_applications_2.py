...
def scrape_detail(id):
    ...
    url = 'https://publicaccess.glasgow.gov.uk/online-applications/applicationDetails.do?activeTab=details&keyVal='+id
    ...
    record['parish'] = root.xpath('.//table//th[contains(text(), "Community Council")]')[0].getnext().xpath('.//text()')[0]
    ...
    if root.xpath('.//table//th[contains(text(), "Agent Phone Number")]'):       
       if root.xpath('.//table//th[contains(text(), "Agent Phone Number")]')[0].getnext().xpath('.//text()') != []:
           record['agent phone number'] =root.xpath('.//table//th[contains(text(), "Agent Phone Number")]')[0].getnext().xpath('.//text()')[0]
       else:
           record['agent phone number'] = 'N/A'
    return record
...
search_url = 'https://publicaccess.glasgow.gov.uk/online-applications/search.do?action=weeklyList'
...
select = br.find_control(name="week")
br.submit()
