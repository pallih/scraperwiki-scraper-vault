...
for el_date in sorted_dates:
    ...
    if '2001' in el_date:
        ...
        # simulate click on 2nd link in the right sidebar 
        #br.open(br.click_link(url_regex='tpel=S&dtel=.*&tpa=G', nr=0))

        area_links = {}
    
        for l in br.links(url_regex='tpel=S&dtel=.*&tpa=G&tpe=A'):
            area_links[l.text] = l.url
    
    
        #sorted_area = reversed(sorted(area_links.iterkeys()))
        sorted_area = sorted(area_links.iterkeys())
                        ...
                        for com in sorted_com:
                            ...
                            for row in rows:
                                ...
                                if len(tds) == 8:
                                    ...
                                    scraperwiki.sqlite.save(unique_keys=['date', 'area', 'reg', 'prov', 'com', 'candidate', 'party', 'votes', 'perc'], data=data)import scraperwiki
import mechanize
import lxml.html           
