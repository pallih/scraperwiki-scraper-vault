...
#words = ['Rackspace']
words = ['__22Amazon+Web+Services__22']
...
words.each do |word|
  ...
  returned_rows = ''
  page = 1
  #url = "http://jobsearch.monster.co.uk/search/?q=" + word + "&sort=dt.rv.di"
  url = "http://jobsearch.monster.co.uk/jobs/?q=" + word + "&sort=dt.rv.di&cy=uk"
  ...
  while url.length > 0
    ...
    doc_rows.each do |vv|
      ...
      if salary_text.length > 2 then
        ...
        for i in 0..1
          sym = salary.inner_text.strip[i] rescue sym = ''  

          salary_text = salary_text.sub(sym, '')
          salary_text = salary_text.sub(sym, '')
          salary_from = salary_from.sub(sym, '')
          salary_to = salary_to.sub(sym, '')
        end
      end
