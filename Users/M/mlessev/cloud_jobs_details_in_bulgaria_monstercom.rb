...
#ScraperWiki::sqliteexecute("delete from cloud_possitions")
#ScraperWiki::sqliteexecute("delete from all_possitions")
#ScraperWiki::sqliteexecute("delete from cloud_job_details where id not in (select id from cloud_possitions)")

#ScraperWiki::sqliteexecute("drop table cloud_job_details")
...
#  url = "http://jobsearch.monster.com/search/?q=" + word + "&sort=dt.rv.di" 
  url = "http://jobsearch.monster.com/jobs/?q=" + word + "&sort=dt.rv.di&cy=bg"
  ...
  while url.length > 0 
    ...
    doc_rows.each do |vv|
      ...
      res = ScraperWiki::select( "count(id) as flag from cloud_job_details where id = '" + job_id + "'") rescue res = []
      exist_flag = 0
      for d in res
        exist_flag = d["flag"]
      end # for
...
returned_rows = ''
page = 1
url = "http://jobsearch.monster.com/jobs/?cy=bg"
...
while url.length > 0 
  ...
  p 'All -> p' + page.to_s + '(' + total_rows.to_s  + ')'
  page += 1

end # while
p 'All -> (saved) ' + total_rows.to_s + ' - ' + returned_rows# Blank Ruby
# encoding: utf-8
...
p 'All -> (saved) ' + total_rows.to_s + ' - ' + returned_rows