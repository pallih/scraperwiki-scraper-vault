...
$html = scraperWiki::scrape('http://www.venus.dti.ne.jp/nekoneko/uso900/uso900.html');
$html = mb_convert_encoding($html, "UTF-8", "SJIS");

$dom = new simple_html_dom();
$dom->load($html);
foreach ($dom->find('body') as $text){ 
    echo $text ."\n";
}
exit;

foreach ($dom->find('a') as $td){ //リンク
    $all_href[] = $td->href;
}
...
foreach($text as $val){
    ...
    if(preg_match($time_reg, $val, $match)){
        ...
        $item[] = array("text" =>$val,"date" => $p_date, "href" => $href[$i++]);
...
foreach($item as $key => $val){
    //$data = array(array("id"=>$key, "text" => $val['text'], "href" => $val['href'], "date" => $val['date']));
