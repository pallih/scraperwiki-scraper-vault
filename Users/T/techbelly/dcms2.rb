...
def find_member_from_url(name, url)
  ...
  member_html = scrape_and_convert_to_utf8(url)
...
def find_lord_from_url(name, url)
  ...
  lords_html = scrape_and_convert_to_utf8(url)
...
def scrape_speech_list(page_body)
  ...
  doc.css('div.ArticleDetail h2 a').each do |speech|
    ...
    the_actual_speech_html = scrape_and_convert_to_utf8(record['permalink'])
...
index_html = scrape_and_convert_to_utf8(starting_url)
