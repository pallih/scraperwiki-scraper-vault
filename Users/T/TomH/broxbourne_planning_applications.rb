...
def get_user_agent
  ...
  agent.get("http://planning.broxbourne.gov.uk/Planning/_javascriptDetector_?goto=/Planning/lg/GFPlanningWelcome.page")
...
def clean_address(address)
  if match = address.match(/ *((?:EN|SG)[0-9]{1,2}) *([0-9][a-z]{2}) *$/i)
  ...
  if total_pages == 10
    ...
    page.root.css("tbody").each do |table|
      ...
      table.css("tr").each do |row|
        ...
        record[:date_validated] = Date.strptime(columns[3].content, "%d/%m/%Y")
    ...
    if current_page < total_pages
      scroller = page.form_with(:id => "_id71")
      scroller["_id71:scroll_1"] = "next"
...
def search_for_applications(to_date = Date.today, from_date = to_date - 14, agent = get_user_agent)
  ...
  search_page = agent.get("http://planning.broxbourne.gov.uk/Planning/lg/plansearch.page?org.apache.shale.dialog.DIALOG_NAME=gfplanningsearch&Param=lg.Planning")
  search_form = search_page.form_with(:id => "_id68")
  search_form["_id68:valid_dateFrom"] = from_date.strftime("%d/%m/%Y")
  search_form["_id68:valid_dateTo"] = to_date.strftime("%d/%m/%Y")
  search_button = search_form.button_with(:id => "_id68:_id81")
...
def update_application(agent, record)
  # Get the search page and submit the search
  search_page = agent.get("http://planning.broxbourne.gov.uk/Planning/lg/plansearchref.page?org.apache.shale.dialog.DIALOG_NAME=gfplanningsearchref&Param=lg.Planning")
  search_form = search_page.form_with(:id => "_id66")
  search_form["_id66:ref_no"] = record["uid"]
  search_button = search_form.button_with(:id => "_id66:_id79")
  search_result = agent.submit(search_form, search_button)

  # Find the table with the details and extract them
  search_result.root.css("table.planSResulttableStyle tbody").each do |table|
    table.css("tr").each do |row|
      ...
      if value.length > 0
        ...
        when "Application Status"; record[:status] = value
