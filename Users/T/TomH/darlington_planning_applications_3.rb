...
def get_user_agent
  ...
  agent.get("http://msp.darlington.gov.uk/Planning/_javascriptDetector_?goto=/Planning/lg/GFPlanningWelcome.page")
  ...
  if total_pages == 10
    ...
    page.root.css("tbody").each do |table|
      ...
      table.css("tr").each do |row|
        ...
        record[:address] = columns[1].content
        record[:ward_name] = columns[2].content
    ...
    if current_page < total_pages
      scroller = page.form_with(:id => "_id116")
      scroller["_id116:scroll_1"] = "next"
...
def search_for_applications(to_date = Date.today, from_date = to_date - 14, agent = get_user_agent)
  ...
  search_page = agent.get("http://msp.darlington.gov.uk/Planning/lg/plansearch.page?org.apache.shale.dialog.DIALOG_NAME=gfplanningsearch&Param=lg.Planning")
  search_form = search_page.form_with(:id => "_id113")
  search_form["_id113:SDate5From"] = from_date.strftime("%d/%m/%Y")
  search_form["_id113:SDate5To"] = to_date.strftime("%d/%m/%Y")
  search_button = search_form.button_with(:id => "_id113:_id182")
...
def update_application(agent, record)
  ...
  search_page = agent.get("http://msp.darlington.gov.uk/Planning/lg/plansearch.page?org.apache.shale.dialog.DIALOG_NAME=gfplanningsearch&Param=lg.Planning")
  search_form = search_page.form_with(:id => "_id113")
  search_form["_id113:SDescription"] = record["uid"]
  search_button = search_form.button_with(:id => "_id113:_id182")
  search_result = agent.submit(search_form, search_button)

  # Look for any list of results in case we get more than one match
  if search_form = search_result.form_with(:id => "_id116")
  ...
  search_result.root.css("table[@title='Case Details'] tbody").each do |table|
    table.css("tr").each do |row|
      ...
      if value.length > 0
        case name
        when "Alternate Reference"; record[:associated_application_uid] = value.upcase
        when "Application Address"; record[:address] = value
        when "Applicant Name"; record[:applicant_name] = value
        when "Agent Name"; record[:agent_name] = value
        when "Agent Address"; record[:agent_address] = value
        ...
        when "Case Type"; record[:application_type] = value
