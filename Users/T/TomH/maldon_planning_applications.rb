...
def get_user_agent
  ...
  agent.get("http://myplan80.maldon.gov.uk/Planning/_javascriptDetector_?goto=/Planning/lg/GFPlanningWelcome.page")
...
def process_search_results(agent, to_date, from_date, page)
  ...
  page.root.css("table.scroller").each do |scroller|
    if current_page = scroller.css("td[@style!='']").first
      current_page = current_page.content.to_i
  ...
  if total_pages == 10
    ...
    page.root.css("tbody").each do |table|
      ...
      table.css("tr").each do |row|
        ...
        record[:decision] = columns[2].content

        if columns[3].content.length > 0
          record[:address] = columns[3].content + ", " + columns[4].content
        else
          record[:address] = columns[4].content
        end
    ...
    if current_page < total_pages
      scroller = page.form_with(:id => "_id60")
      scroller["_id60:scroll_1"] = "next"
...
def search_for_applications(to_date = Date.today, from_date = to_date - 14, agent = get_user_agent)
  ...
  search_page = agent.get("http://myplan80.maldon.gov.uk/Planning/lg/plansearch.page?org.apache.shale.dialog.DIALOG_NAME=gfplanningsearch&Param=lg.Planning")
  search_form = search_page.form_with(:id => "_id61")
  search_form["_id61:SDate1From"] = from_date.strftime("%d/%m/%Y")
  search_form["_id61:SDate1To"] = to_date.strftime("%d/%m/%Y")
  search_button = search_form.button_with(:id => "_id61:_id123")
...
def update_application(agent, record)
  ...
  search_page = agent.get("http://myplan80.maldon.gov.uk/Planning/lg/plansearch.page?org.apache.shale.dialog.DIALOG_NAME=gfplanningsearch&Param=lg.Planning")
  search_form = search_page.form_with(:id => "_id61")
  search_form["_id61:SDescription"] = record["uid"]
  search_button = search_form.button_with(:id => "_id61:_id123")
  ...
  search_result.root.css("table[@title='Address Details'] tbody").each do |table|
    ...
    table.css("tr").each do |row|
      ...
      if value.length > 0
        ...
        when "Address 1", "Street Name", "Address 3", "Address 4", "Address 5"; address.push(value)
  ...
  search_result.root.css("table[@title='Case Details'] tbody").each do |table|
    table.css("tr").each do |row|
      ...
      if value.length > 0
        case name
        when "Alternate Reference"; record[:planning_portal_id] = value
