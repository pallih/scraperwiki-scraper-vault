...
def get_user_agent
  ...
  agent.get("http://planning.denbighshire.gov.uk/_javascriptDetector_?goto=/lg/GFPlanningWelcome.page")
...
def clean_address(address)
  if match = address.match(/ *((?:LL|CH)[0-9]{1,2}) *([0-9][a-z]{2}) *$/i)
  ...
  if total_pages == 10
    ...
    page.root.css("tbody").each do |table|
      ...
      table.css("tr").each do |row|
        ...
        record[:description] = columns[1].content
        record[:address], record[:postcode] = clean_address(columns[2].content)
        record[:date_received] = Date.strptime(columns[3].content, "%d/%m/%Y")
    ...
    if current_page < total_pages
      scroller = page.form_with(:id => "_id146")
      scroller["_id146:scroll_1"] = "next"
...
def search_for_applications(to_date = Date.today, from_date = to_date - 14, agent = get_user_agent)
  ...
  search_page = agent.get("http://planning.denbighshire.gov.uk/lg/plansearch.page?org.apache.shale.dialog.DIALOG_NAME=gfplanningsearch&Param=lg.Planning")
  search_form = search_page.form_with(:id => "_id143")
  search_form["_id143:valid_dateFrom"] = from_date.strftime("%d/%m/%Y")
  search_form["_id143:valid_dateTo"] = to_date.strftime("%d/%m/%Y")
  search_button = search_form.button_with(:id => "_id143:_id155")
