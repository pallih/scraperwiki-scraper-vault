...
def get_user_agent
  ...
  agent.get("http://planportal.erewash.gov.uk/PlanningLive/_javascriptDetector_?goto=/PlanningLive/lg/GFPlanningWelcome.page")
...
def clean_address(address)
  if match = address.match(/ *((?:DE|NG)[0-9]{1,2}) *([0-9][a-z]{2}) *$/i)
  ...
  if total_pages == 10
    ...
    page.root.css("tbody").each do |table|
      ...
      table.css("tr").each do |row|
        ...
        record[:date_validated] = Date.strptime(columns[1].content, "%d/%m/%Y")
        record[:address], record[:postcode] = clean_address(columns[2].content)
        record[:description] = columns[3].content
        record[:decision] = columns[4].content if columns[4].content.length > 0
        record[:decision_date] = Date.strptime(columns[5].content, "%d/%m/%Y") if columns[5].content.length > 0
    ...
    if current_page < total_pages
      scroller = page.form_with(:id => "_id49")
      scroller["_id49:scroll_1"] = "next"
...
def search_for_applications(to_date = Date.today, from_date = to_date - 14, agent = get_user_agent)
  ...
  # Work out the from and to dates relative to the reference date
  to = to_date < fw ? "fw-#{(fw - to_date).to_i}" : "fw+#{(to_date - fw).to_i}"
  from = from_date < fw ? "fw-#{(fw - from_date).to_i}" : "fw+#{(from_date - fw).to_i}"

  # Do the search
  search_results = agent.get("http://planportal.erewash.gov.uk/PlanningLive/lg/plansearch.page?org.apache.shale.dialog.DIALOG_NAME=gfplanningsearch&Param=lg.Planning&ResponseDateFrom=#{from}&ResponseDateTo=#{to}")
...
def update_application(agent, record)
  ...
  search_page = agent.get("http://planportal.erewash.gov.uk/PlanningLive/lg/plansearch.page?org.apache.shale.dialog.DIALOG_NAME=gfplanningsearch&Param=lg.Planning")
  search_form = search_page.form_with(:id => "_id44")
  search_form["_id44:LARef"] = record["uid"]
  search_button = search_form.button_with(:id => "_id44:_id63")
  search_result = agent.submit(search_form, search_button)

  # Select the right result if we got more than one match
  if search_form = search_result.form_with(:id => "_id49")
    search_button = search_form.button_with(:value => record["uid"])
    search_result = agent.submit(search_form, search_button)
  end
  ...
  search_result.root.css("table[@title='Address Details'] tbody").each do |table|
    ...
    table.css("tr").each do |row|
      ...
      if value.length > 0
        case name
        when "House Name", "House Number", "Address Line 1", "Address Line 2", "Address Line 3"; address.push(value)
  ...
  search_result.root.css("table[@title='Case Details'] tbody").each do |table|
    table.css("tr").each do |row|
      ...
      if value.length > 0
        case name
        when "Web Search Reference"; record[:reference] = value
        when "Application Description"; record[:description] = value
        when "Application Type"; record[:application_type] = value
        when "Officer"; record[:case_officer] = value
        when "Valid Date"; record[:date_validated] = Date.strptime(value, "%d/%m/%Y")
        when "Consultee Finish Date"; record[:consultation_end_date] = Date.strptime(value, "%d/%m/%Y")
        when "8 Week Determination Date"; record[:target_decision_date] = Date.strptime(value, "%d/%m/%Y")
        end
      end
    end
  end

  # Find the table with the decision details and extract them
  search_result.root.css("table[@title='Decision Details'] tbody").each do |table|
    table.css("tr").each do |row|
      name = row.css("td.planSResultcol1").first.content
      value = row.css("td.planSResultcol2").first.content
      ...
      if value.length > 0
        ...
        when "Decision Expiry Date"; record[:permission_expires_date] = Date.strptime(value, "%d/%m/%Y")
        end
      end
    end
  end
