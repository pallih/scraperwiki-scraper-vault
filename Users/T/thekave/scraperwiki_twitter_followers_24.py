...
url = 'http://api.twitter.com/1.1/followers/ids.json?screen_name=%s' % (urllib2.quote(SCREENNAME))
print url
...
for follower_list in group(followers_json, 100):
    ...
    url = 'http://api.twitter.com/1.1/users/lookup.json?user_id=%s' % (urllib2.quote(','.join(map(str, follower_list))))
