...
MAX_KM_FROM_PROVINCE = 900
MAX_KM_FROM_COUNTRY = 2000
...
PREVIOUS_SCRAPER = 'geocode_all_mix_scrapers_3'

def geocode():
    if "scraped" not in show_tables():
        d = swimport('csv2sw').read.csv('http://hacks.thomaslevine.com/all_except_standardbankforex.csv')
        save([], d, 'scraped')
...
, coalesce(`latitude-scraped`, `latitude-geocode`) AS latitude
, coalesce(`longitude-scraped`, `longitude-geocode`) AS longitude 
...
, `latitude-scraped` AS latitude
, `longitude-scraped` AS longitude
...
print "Joining"
#execute('DROP TABLE `branch_best-address`')
#choose()
joinbest()
