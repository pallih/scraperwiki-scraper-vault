...
class Faculty(Get):
    def parse(self, text):
        ...
        save(['url'], {"url": self.url, "plaintext": html.get_element_by_id('maincol').text_content()}, 'maincol')
...
def find_similar_research():
    research = select('url, plaintext as "value" from maincol where url != ?;', [reference_person])
    research.extend(select('url, plaintext as "value" from maincol where url = ?;', [reference_person]))
    ...
    save(['url'], [{"url": row[0], "similarity": row[1][1]} for row in zip([row['url'] for row in research], list(enumerate(sims)))], 'similarity')
