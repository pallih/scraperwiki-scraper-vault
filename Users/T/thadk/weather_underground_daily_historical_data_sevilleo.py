        ...
        for column in row.split(","):
            if rowNum == 0:
                ...
                if columnNum <= len(headers) - 1:
                    if headers[columnNum] == 'SolarRadiationWatts/m^2':
                        headers[columnNum] = 'SolarRadiationWattsm2'
...
startt = datetime.fromtimestamp(1332734400)
endt = datetime.fromtimestamp(1357016400)
current = startt
while (current <= endt):
    print current
    time.sleep(2)
    getHistoricalData("KOHBARBE1", current.timetuple()[0], current.timetuple()[1], current.timetuple()[2])
