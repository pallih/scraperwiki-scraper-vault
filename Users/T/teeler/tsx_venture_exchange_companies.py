...
def scrape_table(root):
    ...
    while i < len(rows):
        ...
        while i < len(rows) and rows[i].get('class') != "oddRow":
            ...
            print company_name + ", " + name + ", " + ticker
            i += 1    
            scraperwiki.datastore.save(["tname"], record) # save the records one by one
...
base_path = urlparse.urljoin(base_url, '/HttpController?' +\
           ...
           'SearchKeyword=%s&SearchType=StartWith&Page=1&SearchIsMarket=Yes&Market=V&Language=en')
