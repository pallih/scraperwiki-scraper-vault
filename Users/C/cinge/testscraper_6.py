...
baseurl = "http://www.fsa.gov.uk/register/firmMainSearch.do?" #string
qtwo = "firmName=&"
...
def savelist():
    parturl = "http://www.fsa.gov.uk/register/firmBasicDetails.do?sid="
    for item in sidlist:
        firmurl = parturl+item
        print firmurl
        scraperwiki.sqlite.save(unique_keys=["Firm"], data={"Firm":firmurl, "SID":item},table_name='Firms')
...
def grabLinks(postcode, pagenumber):
    try:
        ...
        for x in range(1,pcount+1):
            ...
            root = lxml.html.fromstring(html)
            for mylink in root.cssselect("table.search-results a"):
                sidlist.append (regFind (mylink.attrib['href']))
    except ValueError:
        scraperwiki.sqlite.save(unique_keys=["Postcode"], data={"Postcode":postcode},table_name='Log')

try:
    scraperwiki.sqlite.execute("create table Firms (Firm string, SID string)")
    scraperwiki.sqlite.execute("create table Log (Postcode string)")
except:
...
for item in postcodelist:
    ...
    savelist()
    sidlist=[] #don't forget to re-initialist the list. Not sure why I put the list in the first place. I thought it was a good idea at the time ;-)
