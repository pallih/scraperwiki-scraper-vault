...
# ** MAKE SURE CYCLE THROUGH PAGES (COULD BE MORE THAN TWO) ***
# ** OPERATE UNDER ASSUMPTION DON'T NEED TRADE NAME / SUPPLIER / PURPOSE; JUST INGREDIENTS, CHEM ABSTRACT, AND MAX INGREDIENTS BOTH WAYS **
# url = "http://fracfocus.ca/find_well/download/BC/10203523" # original URL, works with this one
# Scraperwiki's PDF to XML tool helpful: https://views.scraperwiki.com/run/pdf-to-html-preview-1/

# http://fracfocus.ca/find_well/download/BC/9725389
# http://fracfocus.ca/find_well/download/BC/9069394

url = "http://fracfocus.ca/find_well/download/BC/9725389" #tried various PDFs, format seems the same at least for top block
...
# need to figure out a way to grab first page 1, then page 2, but for now going to try to figure using top as guide; entire page seems to be within a page tag

# try grabbing just second page

page2 = root.find('.//page[@number="1"]')
...
#pagedata = page2.findall('.//text[@font="4"]')

#for text in pagedata:
#    print text.text
...
while xpathtopvalue < 1000:
    record["Ingredients"] = ""
    record["ChemAbstract"] = ""
    record["MaxIngredientAdd"] = ""
    record["MaxIngredientConc"] = ""    
    xpathtopvalue = xpathtopvalue + 1
    xpathtext = './/text[@top="' + str(xpathtopvalue) + '"]'
    texttags = page2.findall(xpathtext)
...
# trying to do this with date/time string so won't reset with new tries
...
'''
    for text in pagedata:
        tagtop = text.attrib.get('top')
        tagtop = int(tagtop)
        left = text.attrib.get('left')
        leftinteger = int(left)
        if tagtop == xpathtopvalue:
'''
#            if leftinteger == 59:
