#attempted to use ordered dict to fix problem or not being able to reverse dict without success 
#problem with follower_list not being defined?
...
print url
#followers_json = simplejson.loads(scraperwiki.scrape(url), object_pairs_hook=collections.OrderedDict) #load json as an ordered dict
followers_json = simplejson.loads(scraperwiki.scrape(url)) 

print "Found %d followers of %s" % (len(followers_json), SCREENNAME)

#items = followers_json.items()
#print items
#items.reverse()
#print items


# get earliest followers first for batching
