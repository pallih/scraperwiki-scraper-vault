...
while j<queries.count
  queryurl='http://dcc.ufrj.br/~diogobor/browse.php?u=http://www.jobisjob.com.br/'+queries[j]+'/vagas/'
 # puts queryurl
  doc = Nokogiri::HTML(open(URI::encode(queryurl)))
doc.search("div[@class='sponsored']").each do |node|
  #puts node
   span=node.search("p[@class='from']").each do |innode|
   cells=innode.search ('span')
    if(cells.count>0)  
   # puts cells.text
    name=cells.inner_html
   # puts name
    name.slice! "De: "  
   # puts name + ''+ queryurl
    data={
