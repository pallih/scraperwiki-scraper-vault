...
for day in range(0,7):
     for chan in range(len(channel_numbers)):
        ...
        for x in range(len(shows)):
            index += 1
            scraperwiki.sqlite.save(unique_keys=["index"], data={"date": offset_date, "time":times[x], "channel":channel_names[chan], "show":shows[x], "index":index, "suffix":suffixes[x]})import scraperwiki
import lxml.html
import datetime
channel_names = ["BBC 1", "BBC 2", "ITV1", "Channel 4", "Channel 5", "BBC Three"]
channel_numbers = [101, 102, 103, 104, 105, 115]
