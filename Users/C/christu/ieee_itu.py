...
# http://www.itu.int/rec/T-REC-B/e
...
limit = 25
# searchURL = 'http://www.itu.int/pub/T-REC'
# 'http://odysseus.ieee.org/query.html?lk=2&style=standard'
baseURL =
# 'http://standards.ieee.org'

 def scrape_search_page(offset):
    html = scraperwiki.scrape(searchURL +'-'+str(offset))
    html = BeautifulSoup(html,convertEntities=BeautifulSoup.HTML_ENTITIES)
    # div = html.find('div') #, { "ctl00_PlaceHolderMain_RichHtmlField1__ControlWrapper_RichHtmlField" : "ms-rtestate-field" })
    links = div.findAll('a', href=re.compile(r'.\recommendation\.asp\?lang=en&parent=.*'))
    for a in links:
...
def scrape_standard_page(url):
    ...
    scraperwiki.sqlite.save(unique_keys=['id'], data = std,table_name="standards_itu_t")
...
#scrape_standard_page('http://standards.ieee.org/findstds/standard/1159-2009.html')# http://odysseus.ieee.org/query.html?lk=2&style=standard&st=26
