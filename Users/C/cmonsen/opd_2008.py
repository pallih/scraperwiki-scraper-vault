...
url = "http://ftp.cdc.gov/pub/Health_Statistics/NCHS/Dataset_Documentation/NAMCS/doc08.pdf"
            ...
            if el.text is not None:
                if el.text.strip().isdigit():
                    ...
                    if splitel[0].isdigit() and int(splitel[0]) < 500:
                        itemn = splitel[0]
                        length = splitel[1]
                        splitel2 = el.getnext().text.strip()
                        range = splitel2[0]
                        label = splitel2[1].replace("[","").replace("]","")
                        point = {"id":itemn,"length":length,"range":range,"label":label}
                        data.append(point)
