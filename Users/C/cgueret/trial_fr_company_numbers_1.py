...
def load_task_queue():
    ...
    '''
    try:
        # Here we don't expect to have the time to deal with more than 5000 tasks per run
        tasks = scraperwiki.sqlite.select("dept,name from tasks where done=0 order by name desc limit 5000")
    except:
        return []
    return tasks
...
# Run the scraper
#
# Initialise the task queue for the first run
#init_task_queue()
go()
