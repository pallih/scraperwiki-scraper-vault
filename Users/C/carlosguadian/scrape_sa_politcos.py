...
def group(lst, n):
    for i in range(0, len(lst), n):
        val = lst[i:i+n]
        yield tuple(val) # want to get last ones out!
...
for follower_list in group(followers_json, 100):
    ...
    for detail in details_json:
        data = {'screen_name': detail['screen_name'],'id': detail['id'],'location': detail['location'], 'bio': detail['description'], 'following':detail['friends_count'],'followers':detail['followers_count'],'tweets':detail['statuses_count'],'favourites':detail['favourites_count'],'verified':detail['verified']}
        assert data['id'] not in ids
        builder.append(data)
        ids.append(data['id'])
    print len(builder)
    scraperwiki.sqlite.save(unique_keys=['id'], data = builder)
