...
// The base URL.
$base_url = "http://southliquormart.com/shop/shop-by-features/wine-club";
...
foreach($html->find("ul[@class='VMmenu215845_1'] li a") as $menu_link){
    ...
    $product_string = $html_1->find("div[@class='category-view'] a");
    $sub_category_string = $html_1->find("h2[@class='catSub'] h2 a");
    ...
    elseif(!empty($sub_category_string)){
        ...
        foreach($sub_category_string as $menu_link_2){
            ...
            // Look for the product links
            foreach($html_2->find("div[@id='mainContent'] a") as $product_link){
                handle_products($product_link);
            }
        }
    }
...
function handle_products($product_link)
{
    ...
    if(!empty($product_link)){
        ...
        if ($found === false) {
            ...
            $cat_raw = str_replace("/shop/", "", $product_link->href);
