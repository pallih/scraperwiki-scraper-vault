"""
DCSF School league tables: Key stage 4 (GCSE and equivalent)

http://www.dcsf.gov.uk/performancetables/schools_09.shtml

The following document explains some of the codes:

http://www.education.gov.uk/performancetables/schools_09/glossary.shtml#z07
"""
...
def get_lea_list():
    ...
    url = "http://www.dcsf.gov.uk/performancetables/schools_09.shtml"
    page = urlopen(url)
...
def obtain_lea_number(lea_name):
    ...
    url = "http://www.dcsf.gov.uk/cgi-bin/performancetables/search_09.pl?Fieldname=&Fieldpostcode=&FieldLEA=%s&Mode=Z&Phase=1&Year=09&Base=b&Begin=s"%"+".join(lea_name.split(" "))
...
def scrape_lea(name, number):
    ...
    url = "http://www.dcsf.gov.uk/cgi-bin/performancetables/group_09.pl?Mode=Z&Type=LA&Begin=s&No=%d&Base=g&Phase=1&F=1&L=10000&Year=09&F=1&L=10000"%number
    page = html.parse(url)
