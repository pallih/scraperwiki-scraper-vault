...
FIXED_SEARCH_PARAMS="accion=Mas"
...
class BOESearchSpider
  ...
  def get_search_info()
    ...
    # BOE REsults page has changed we need to recompute the way we extract id_busqueda
    #id_busqueda = results.form.field_with(:name => 'id_busqueda').value
    href_idBusqueda = @agent.page.parser.css('ul.navlist li a.current')[0]["href"] =~ /&id_busqueda=(.*?)-/
    id_busqueda = $1
    p id_busqueda
  ...
  def fetch_results(id,start)
    params = "&id_busqueda=#{id}-#{start}-#{MAX_PAGE_HITS}"
...
spider.get_search_results()## BOE SEARCH RESULTS SCRAPER ##
# scraping libraries
require 'mechanize'
require 'nokogiri'
require 'open-uri'
