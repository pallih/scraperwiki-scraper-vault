...
# North East Planning Applications

scrape_lib = scraperwiki.utils.swimport("pa_scraper_library")

REGION = 'North East'
DB_START = 1 # default date (in YYYY-MM-DD format) to scrape from (or a number if you want it to start x days ago)
...
NUM_PER_DAY = 50 # number of authorities updated on each run
...
#scrape_lib.restart_authority('Preston')

#uncomment these lines to to do a short test scrape on one authority
#auth_list = scrape_lib.get_auths(['Northumberland']) # NB simple string is a region, list is a list of authorities
#scrape_lib.gather_applications(auth_list[0], DB_START, DB_END, 100, 240)
