...
while exists == True:
    html = scraperwiki.scrape("http://www.oscn.net/applications/oscn/getcaseinformation.asp?query=true&srch=0&web=true&db=Tulsa&number=cf-2013-" + str(counter) + "&iLAST=&iFIRST=&iMIDDLE=&iID=&iDOBL=&iDOBH=&SearchType=0&iDCPT=&iapcasetype=All&idccasetype=All&iDATEL=&iDATEH=&iCLOSEDL=&iCLOSEDH=&iDCType=0&iYear=&iNumber=&icitation=&submitted=true")
    ...
    if len(root.cssselect("table")) != 8: # Check if page is a case record page or the 404 page (record pages have 8 tables, errors have 11)
        ...
        data = {        
               'case_number' : "cf-2013-" + str(counter),
               'name' : name,
               'judge' : judge,
               'county' : "Tulsa",#   may change
               'parties' : parties,
               'attornies' : attornies,
               'counts' : counts,
               }
