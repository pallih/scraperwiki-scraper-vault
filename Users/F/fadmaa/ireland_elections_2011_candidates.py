...
def scrapeCandidate(key,txt):
  ...
  soup = BeautifulSoup(html)
  divs1 = soup.find('div',{'id':'sort_holder'}).findAll('div',{'class':'post postDiscussed-candidate span-4'})
  divs2 = soup.find('div',{'id':'sort_holder'}).findAll('div',{'class':'post postDiscussed-candidate span-4 last'})
  scrapeDivs(divs1,key,txt)
  scrapeDivs(divs2,key,txt)

def scrapeDivs(divs,key,txt):
  for div in divs:
    a = div.find('div',{'class':'text'}).find('a')
    """img = div.find('div',{'class':'img'}).find('img')['src']
    tmp = div.find('div',{'class':'constituency'})
    prty = tmp.find('a')
    cons = prty.findNextSiblings('a')[0]
    record = {}
    record['name'] = a.text
    record['party'] = prty.text
    record['constituency'] = cons.text
    record['img_url'] = img
    record['url'] = a['href']
    scraperwiki.datastore.save(['url','img_url','party'], record)"""
    getDetails(a['href'] + '/')
...
getDetails("http://www.thejournal.ie/election-candidates-2011/wicklow/dick-roche/")import scraperwiki
import time 
import random 
...
"""for i in range(42,43):
  ...
  scrapeCandidate(u['href'][2:]+'/',u['text'])
  time.sleep(random.uniform(1, 4) )"""
getDetails("http://www.thejournal.ie/election-candidates-2011/wicklow/dick-roche/")