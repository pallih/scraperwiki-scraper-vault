...
def scrape_member(id):
    ...
    committee_memb_table = details_tables[0]
    print "ctte member table"
    committee_sub_table = details_tables[1]
    committee_memberships = get_memberships(committee_memb_table)
...
members = []
for m in index_page.find("td", {"class":"box_content_mep"}).findAll("td", {"class":"listcontentlight_left"})+index_page.find("td", {"class":"box_content_mep"}).findAll("td", {"class":"listcontentlight_left"}):
    s = m.find("a")["href"]
    start = s.find("&id=")+4
    id = int(s[start:])
    scrape_member(id)
