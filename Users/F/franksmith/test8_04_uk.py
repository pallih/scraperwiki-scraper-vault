...
def BuildActiveList():
    ...
    existingList = []
    existingDict = {}
    scraperwiki.sqlite.attach("test6uk")
    activeDict = scraperwiki.sqlite.select("auctionNumber from test6uk.newTable")
    print activeDict
...
#    itemMakeOfferTag = "//a[@id='but_v4-2'] | //a[@id='but_v4-3']"      #Only for active make offer-- found but_v4-3 id on Buy Now on Sale with Make Offer
    ...
    url = 'http://www.ebay.co.uk/itm/' + inItemNumber
    invalidread = 1
    invalidreadcount = 1
...
### Check first if it's removed or not; or if page is not available
    try:
