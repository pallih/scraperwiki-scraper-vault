...
def scrapepdf(pdfurl):
    ...
    for idx, text in enumerate(s.findAll('text')):
        ...
        if -1 != msg.find("Votering i sak nr."):
            ...
            continue
        if -1 != msg.find("enstemmig bifalt") or -1 != msg.find("enstemmig vedtatt") or ((-1 != msg.find("bifalt") or -1 != msg.find("vedtatt")) and -1 != last_line.find("ble enstemmig")):         
            ...
            if casenr is not None:
                scraperwiki.sqlite.save(unique_keys=['date', 'casenum', 'index'], data=data)
        last_line = msg
