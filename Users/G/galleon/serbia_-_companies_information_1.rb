...
def action(srch)
  ...
  begin
    ...
    params = {
     "SelectedRegisterId"=>"",
     "SearchByRegistryCodeString"=>"#{srch}__",
     "SearchByNameString"=>"",
...
if srch == "<non-alpha>"
  range = (0..9)
  offset = get_metadata("OFFSET",00000000)
  range.each_with_index{|srch,idx|
    next if idx < offset
    action(srch)
    save_metadata("OFFSET",idx.next)
  }
  save_metadata("TRIAL","00000000")
...
begin
  ...
  if ret.nil? or ret == 0
    if trial.nil? or trial.empty? 
      ...
      t_a = trial.split(">>")
      t_s = t_a.pop
      t_s = t_a.pop until t_s=='9' or not t_s.split(//).last == '9'
      srch = (t_s == '9')? nil : t_s.next
      trial = (t_a << srch).join(">>")
    end
  elsif ret == 101
    srch = srch+'0'
