...
make_url(2,24)import scraperwiki           
import lxml.html
import random
import sys, traceback
import uuid
...
def parse_apt(url):
    try:
        ...
        for i in url:
            ...
            print "Parsing url # ", i
            print i
            if i: ###Take away if it does not pass 538 data rows and 613 sources
                html = scraperwiki.scrape(i)
                root = lxml.html.fromstring(html)
                ammenities =[]
                caracteristics = []
                ...
                for el in root.cssselect("div.caracteristicas.grid_10.alpha.pan.mtl dt"):
                    if el: 
                        ...
                        caracteristics.append(" chars empty")    
                    #print el.text_content().strip()
                for el in root.cssselect("div.caracteristicas.grid_10.alpha.pan.mtl dd"):
                    vals_caracteristics.append(el.text_content().strip())
                    #print el.text_content().strip()
...
make_url(1,24)import scraperwiki           
import lxml.html
import random
import sys, traceback
import uuid
