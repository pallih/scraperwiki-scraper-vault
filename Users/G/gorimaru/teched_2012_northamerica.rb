...
def parse_doc(doc)
  ...
  doc.xpath('//div[@class="entry-meta"]').each do |entry|
    ...
    downloads = entry.xpath('ul[@class="downloads"]/li/ul/li[@class="slides"]/a/@href')
...
urls = (1..3).map {|i| "http://channel9.msdn.com/Events/TechEd/NorthAmerica/2012?sort=status&direction=asc&page=#{i}" }
urls.each do |url|
  puts "Fetching #{url}"
