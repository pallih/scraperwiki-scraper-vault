...
participants = scraperwiki.sqlite.select("country, song, singer, 0 'mentions', semi from [participants_-_advanced].swdata where semi = 2")
...
for part in part_sorted:
    ...
    results = part['mentions']
        ...
        #if len(stack) > 0:
            #points = stack.pop()
    scraperwiki.sqlite.save(unique_keys=["country"], data={"country":country, "singer":singer, "points":results, "timestamp":run_time})
    print country.encode('utf_8') + " - " + singer.encode('utf_8') + " - " + song.encode('utf_8')+ " - " + str(results) + " - " + str(total_tweets)
