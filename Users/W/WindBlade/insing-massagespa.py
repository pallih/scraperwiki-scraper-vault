...
##starting_url = 'http://search.insing.com/business/world-spa-health-club/orchard-river-valley/id-64110000?#title=Beauty+%26+Spa+%3E+Massage+%26+Spas&refinedCategory=Beauty+%26+Spa+%3E+Massage+%26+Spas&taxonomyNode=Beauty+%26+Spa+%3E+Massage+%26+Spas&shuffling_cipher=4&businessPage=1&resultIndex=1'
#
###############################################################################
import urllib2
import scraperwiki
...
def scrape_table(soup, url):
    ...
    address = soup.find("p", { "class" : "address"}).renderContents().strip()##p for paragraph
    tel = ""
    fax = ""
    hours = ""
    cat = ""
    ...
    id1 = url.find('id-')##what is all of this?
    id = url[id1+3:id1+11]
    ...
    if phones:
        ...
        if p:
            ...
            low = telRaw.find("e64('")
            high = telRaw.rfind("'))")
            tel = base64.decodestring(telRaw[low+5:high])## what is this?
...
starting_url ='http://search.insing.com/business/a-company-for-women/tanglin-dempsey-orchard/id-577a0000?title=Medical+%26+Dental+Services+%3E+General+Practitioners&sortBy=a%3AEXACT_TRADING_NAME&refinedCategory=Medical+%26+Dental+Services+%3E+General+Practitioners&taxonomyNode=Medical+%26+Dental+Services+%3E+General+Practitioners&shuffling_cipher=13&maxIterations=1&businessPage=1&resultIndex=1'
