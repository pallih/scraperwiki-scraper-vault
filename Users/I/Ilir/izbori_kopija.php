...
<h2><center>Lokalni izbori 2013. (anketa)<center/></h2>
  <br>
  <h4>*Anketa ne dozvoljava upis osobnih podataka. U prvo polje sa zvjezdicom upišite (velikim slovima) naziv županije, u drugo polje upišite (velikim slovima) ime grada. Primjeri su: ISTARSKA ŽUPANIJA i BALE - VALLE, ŠIBENSKO-KNINSKA ŽUPANIJA i CIVLJANE, itd. Iznimku čine GRAD ZAGREB i ZAGREB. Za točan naziv županija i gradova pogledajte pod područja (20 more » područja) <a href="https://scraperwiki.com/scrapers/liste"/>ovdje</a></h4><br>
<form name="input" action="" method="get" onsubmit="return validateForm()">
  <fieldset> <legend>biraci.uprava.hr</legend>
    ...
    <br>
    <input type="text" id="captcha1" name="captcha1" value="'.$captcha1.'" style="'.$captcha1_style.'"/><label for="captcha1"> *prepiši tekst (uputa na vrhu)</label>
    <div>
      <img src="'.$image1.'"/>
    </div>
  </fieldset>
  ...
  <fieldset> <legend>oib.oib.hr</legend>
    ...
    <br>
    <input type="text" id="captcha2" name="captcha2" value="'.$captcha2.'" style="'.$captcha2_style.'"/><label for="captcha2"> *prepiši tekst (uputa na vrhu)</label>
  ...
  <fieldset> <legend>nacionalne manjine</legend>
    <br style="margin: 4px; display:block"/>
    <input type="text" id="manjina" name="manjina" value="'.$minor.'"/><label for="manjina"> manjina</label>
    <br><br>
    ...
    for ($j=0; $j < $idx; $j++) {
        ...
        while ($line !== false) {
            ...
            if ($code == 77 || $code == 189 || $code == 32) {
                if ($hist == 32) {
                    $pdx == 0 ? $persy[$j][$pdx] .= $line : $persy[$j][$pdx-1] .= $line;
                } else {
                    $persy[$j][$pdx++] = $line;
                }
            }
