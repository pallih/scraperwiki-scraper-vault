...
class HealthScraper
    ...
    loop do
      Nokogiri::HTML( index_page.body ).search('#ResultsDataGrid tr').each do |row|
        if restaurant_data = parse_index_row(row)
          coordinates = load_cached_coordinates_for(restaurant_data[:id])
          restaurant_data.merge!(coordinates) unless coordinates.nil? 
  ...
  def load_cached_coordinates_for(restaurant_id)
    coordinates = ScraperWiki.select("latitude, longitude FROM restaurant_coordinates WHERE restaurant_id = '#{restaurant_id}'")
  ...
  def geocode_restaurants
    ...
    restaurants.each do |restaurant|
      coordinates = load_cached_coordinates_for(restaurant['id'])
...
scraper = HealthScraper.new
scraper.fetch_restaurants
scraper.geocode_restaurantsrequire 'mechanize'
require 'nokogiri'
require 'json'
require 'uri'
require 'open-uri'
