...
scraperwiki.sqlite.execute("CREATE TABLE firstsetd ('Full name' string,'Degree' string,'Title' string, 'Department' string, 'Speciality' string,'Education' string, 'cnt' int)")
    ...
    for link2 in link:
        ...
        if cnt >= 101 :
            if cnt <= 677:
                ...
                print 'last three values are %s    #### %s ##### %s' %(prim2,pa2,ph2)
                
                scraperwiki.sqlite.execute("insert into firstsetd values (?,?,?,?,?,?,?)", (fn,deg,title,depart,spcl,educat,cnt))
                scraperwiki.sqlite.commit()
