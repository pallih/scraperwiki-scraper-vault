...
scraperwiki.sqlite.execute("CREATE TABLE completedrexel3 ('Full name' string,'Degree' string,'Title' string, 'Department' string, 'Speciality' string,'Education' string,'Fellowship' string,'Residency' string, 'Internship' string,'Primary Practice name' string, 'Primary Practice address' string,'Phone'  string,'Cnt' int)")
    ...
    for link2 in link:
        ...
        if cnt >= 101 :
            if cnt <= 677:
                ...
                scraperwiki.sqlite.execute("insert into completedrexel3 values (?,?,?,?,?,?,?,?,?,?,?,?,?)", (fn,deg,title,depart,spcl,educat,fellow,resi,intern,prim2,pa2,ph2,cnt))
                scraperwiki.sqlite.commit()
