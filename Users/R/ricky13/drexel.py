...
scraperwiki.sqlite.execute("CREATE TABLE FinalDanda ('Full name' string,'Degree' string,'Title' string, 'Department' string, 'Speciality' string,'Education' string)")
    ...
    for link2 in link:
        ...
        if cnt >= 101 :
            if cnt <= 677:
                ...
                scraperwiki.sqlite.execute("insert into FinalDanda values (?,?,?,?,?,?)", (fn,deg,title,depart,spcl,educat))
                scraperwiki.sqlite.commit()
