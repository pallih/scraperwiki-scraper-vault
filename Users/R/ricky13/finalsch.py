...
#scraperwiki.sqlite.execute("CREATE TABLE gandu1 ('fi' string)")
...
for l in list:
    ...
    tab = Soup.findAll(name = 'table', attrs = {"class": " address-data"})
    print 'len is %d\n' %(len(tab))
    #print tab
    if len(tab) == 0 :
        text = 'Null$$$$NULL$'
        ...
        t1 = tab[0]
        #print t1
        t2 = t1.findAll(name = 'span', attrs = {"class": "effective"})
        #t2 = t1.findAll(name = 'span')
        #print t2
        #print len(t2)
        #print 't2'
        text = t2[1].renderContents().strip()
        print text
        if len(t2) > 0:
            tay = t2[0].renderContents().strip()
            #tay = t2[0]
            #print tay
            text =  tay.strip()
    ...
    se = text[text.rfind('$')+1:]
    #m = re.search("\n", text)
    #text[m.start()] = '#'
    #print text
    print '%s    $  %s' %(fi,se)
    #print se
    #scraperwiki.sqlite.execute("insert into gandu1 values (?)",(fi))
    #scraperwiki.sqlite.commit()
