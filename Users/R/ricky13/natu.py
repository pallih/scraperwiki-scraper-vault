...
scraperwiki.sqlite.execute("CREATE TABLE natua ('mail' string)")
...
for l in list:
    ...
    if len(tab) == 1 :
        ...
        t2 = t1.findAll(name = 'span', attrs = {"class": "effective"})[0].renderContents().strip()
        text =  t2.strip()
        ...
        text = text.replace(',',';')
        text = text.replace('\s+','')
    scraperwiki.sqlite.execute("insert into  natua values (?)",(text))
    scraperwiki.sqlite.commit()
    print text
