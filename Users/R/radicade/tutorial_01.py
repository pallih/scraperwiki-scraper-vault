import scraperwiki
from scraperwiki.sqlite import save
from urllib2 import urlopen
from lxml.html import fromstring, tostring
import datetime

COLUMN_NAMES = [
    'employer', 'download', 'location', 'union',
    'local', 'naics', 'num_workers', 'expiration_date'
]

STATES = [
    'AL', 'AK', 'AZ', 'AR', 'CA', 'CO', 'CT', 'DE', 'FL', 'GA',
    'HI', 'ID', 'IL', 'IN', 'IA', 'KS', 'KY', 'LA', 'ME', 'MD',
    'MA', 'MI', 'MN', 'MS', 'MO', 'MT', 'NE', 'NV', 'NH', 'NJ',
    'NM', 'NY', 'NC', 'ND', 'OH', 'OK', 'OR', 'PA', 'RI', 'SC',
    'SD', 'TN', 'TX', 'UT', 'VT', 'VA', 'WA', 'WV', 'WI', 'WY'
]

def scrapetable(url):
    download = urlopen(url)
    raw = download.read()
    
    html = fromstring(raw)
    table = html.cssselect('table')[2]
    #for tr in table.cssselect('tr'):
    #tableindex = [1:length(table)]
    for tr in table.cssselect('tr')[1:]:
        #for td  in tr.cssselect('td'):
        #    print td.text_content()
        #print COLUMN_NAMES
        print [td.text_content() for td in tr.cssselect('td')]
        cell_text = [td.text_content() for td in tr.cssselect('td')]
        #print zip(COLUMN_NAMES, cell_text)
        data = dict(zip(COLUMN_NAMES, cell_text))
        #save([], data)
        #Data Cleansing
        #print data['num_workers']
        print data['num_workers']
        #data['num_workers'] = int(data['num_workers'])
        #print data['num_workers']
        if data['location'][:2] in STATES:
            data['state'] = data['location'][:2]
        #print datetime.datetime.strptime(
        #    data['expiration_date'],
        #    '%m-%d-%y'
        #)
        #data['expiration_date'] = datetime.datetime.strptime(data["expiration_date"],"%m-%d-%y").date()
        #print data
        a_elements = tr.cssselect('a')
        if len(a_elements) > 1:
            raise ValueError('Row has multiple a tags.')
        elif len(a_elements) == 1:
            data['pdf'] = 'http://www.dol.gov/olms/regs/compliance/cba/' + a_elements[0].attrib['href']
        elif len(a_elements) == 0:
            pass
        #print data
        save([], data)

#urls = [ "http://www.dol.gov/olms/regs/compliance/cba/Cba_CaCn.htm",
#  "http://www.dol.gov/olms/regs/compliance/cba/Cbau_mamh.htm",
#  "http://www.dol.gov/olms/regs/compliance/cba/Cba_NfOz.htm" ]

#urls = ['http://www.dol.gov/olms/regs/compliance/cba/Cba_CaCn.htm']
urls = ['http://www.dol.gov/olms/regs/compliance/cba/Cbau_mamh.htm']


for url in urls: 
    #scrapetable('http://www.dol.gov/olms/regs/compliance/cba/Cba_CaCn.htm')
    scrapetable(url)


#print urlopen('http://www.dol.gov/olms/regs/compliance/cba/olms/private/3311.pdf').read()
#print raw_pdf
#print pdftoxml(raw_pdf)import scraperwiki
from scraperwiki.sqlite import save
from urllib2 import urlopen
from lxml.html import fromstring, tostring
import datetime

COLUMN_NAMES = [
    'employer', 'download', 'location', 'union',
    'local', 'naics', 'num_workers', 'expiration_date'
]

STATES = [
    'AL', 'AK', 'AZ', 'AR', 'CA', 'CO', 'CT', 'DE', 'FL', 'GA',
    'HI', 'ID', 'IL', 'IN', 'IA', 'KS', 'KY', 'LA', 'ME', 'MD',
    'MA', 'MI', 'MN', 'MS', 'MO', 'MT', 'NE', 'NV', 'NH', 'NJ',
    'NM', 'NY', 'NC', 'ND', 'OH', 'OK', 'OR', 'PA', 'RI', 'SC',
    'SD', 'TN', 'TX', 'UT', 'VT', 'VA', 'WA', 'WV', 'WI', 'WY'
]

def scrapetable(url):
    download = urlopen(url)
    raw = download.read()
    
    html = fromstring(raw)
    table = html.cssselect('table')[2]
    #for tr in table.cssselect('tr'):
    #tableindex = [1:length(table)]
    for tr in table.cssselect('tr')[1:]:
        #for td  in tr.cssselect('td'):
        #    print td.text_content()
        #print COLUMN_NAMES
        print [td.text_content() for td in tr.cssselect('td')]
        cell_text = [td.text_content() for td in tr.cssselect('td')]
        #print zip(COLUMN_NAMES, cell_text)
        data = dict(zip(COLUMN_NAMES, cell_text))
        #save([], data)
        #Data Cleansing
        #print data['num_workers']
        print data['num_workers']
        #data['num_workers'] = int(data['num_workers'])
        #print data['num_workers']
        if data['location'][:2] in STATES:
            data['state'] = data['location'][:2]
        #print datetime.datetime.strptime(
        #    data['expiration_date'],
        #    '%m-%d-%y'
        #)
        #data['expiration_date'] = datetime.datetime.strptime(data["expiration_date"],"%m-%d-%y").date()
        #print data
        a_elements = tr.cssselect('a')
        if len(a_elements) > 1:
            raise ValueError('Row has multiple a tags.')
        elif len(a_elements) == 1:
            data['pdf'] = 'http://www.dol.gov/olms/regs/compliance/cba/' + a_elements[0].attrib['href']
        elif len(a_elements) == 0:
            pass
        #print data
        save([], data)

#urls = [ "http://www.dol.gov/olms/regs/compliance/cba/Cba_CaCn.htm",
#  "http://www.dol.gov/olms/regs/compliance/cba/Cbau_mamh.htm",
#  "http://www.dol.gov/olms/regs/compliance/cba/Cba_NfOz.htm" ]

#urls = ['http://www.dol.gov/olms/regs/compliance/cba/Cba_CaCn.htm']
urls = ['http://www.dol.gov/olms/regs/compliance/cba/Cbau_mamh.htm']


for url in urls: 
    #scrapetable('http://www.dol.gov/olms/regs/compliance/cba/Cba_CaCn.htm')
    scrapetable(url)


#print urlopen('http://www.dol.gov/olms/regs/compliance/cba/olms/private/3311.pdf').read()
#print raw_pdf
#print pdftoxml(raw_pdf)