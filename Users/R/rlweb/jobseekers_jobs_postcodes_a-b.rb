...
def record_job_details(job_link, full_or_part_time)
  ...
  length = record['location'].gsub(/\w\w?\d\d?\w?$/, "").strip.length

  record['postcode'] = record['location'][length..-length] # => length 
...
ScraperWiki.select('* from `uk_postcode_districts` WHERE `postcode_district` LIKE "BS%" Limit 6 Offset 10').each do |district|
  if POSTCODE_PREFIXES.include?(district['postcode_district'][0,2])
    (0..9).each do |num|
      ...
      ["30", "40", "60"].each do |hours|
        ...
        begin
          ...
          search_page.form_with(:name => 'Homepage') do |f|
            ...
            f['ddlDistance'] = "2" # jobs within 5 miles
            f['ddlAge'] = "3" # "today's jobs"
            f['ddlHours'] = hours 
            f['btnSearch'] = "Search"
            results = f.submit()
