...
def get_policy(root):
    try:
        content = root.cssselect('table')[41].text_content()

        if('General Hotel Information:' in content):

            if ( 'Pet Policy:' in content):
                return content[content.find('Pet Policy:') + 12:]
            else:
                return 'Empty Policy (Pet policy not in 41)'
        else:
            content = root.cssselect('table')[32].text_content()
            if('General Hotel Information:' in content):
                ...
                return 'Empty Policy (Gen Info not in 41 and 32)'
