...
for list_mo in re.finditer(r'<a href="(/biographies[^"]+)"', list_page):
  found = True
  mp_url = "http://www.parliament.uk" + list_mo.group(1)
  try:
  ...
  name_mo = re.search(r'<h1 id="ctl00_ctl00_SiteSpecificPlaceholder_.+_h1Item">(?:<!--(?:[^-]|-[^-])+-->|\s)+(.+?)(?:<!--(?:[^-]|-[^-])+-->|\s)+</h1>', mp_page, re.DOTALL)
  if not name_mo:
  ...
  # Constituency
  constituency_mo = re.search(r'<h3[^>]*>Constituency</h3>\s+<p>([^<]+)</p>', mp_page, re.DOTALL)
  ...
  westminster_mo = re.search(r'<strong>Westminster</strong>\s*<p>((?:[^<]+|<[^/]|</[^p])*)</p>', mp_page)
  if not westminster_mo:
    logging.error("Could not find Westminster contact details for %s", mp_url)
    continue
  westminster_contact_details = extract_contact_details(westminster_mo.group(1))
  
  # Get constituency contact details
  constituency_mo = re.search(r'<strong>Constituency</strong>\s*<p>((?:[^<]+|<[^/]|</[^p])*)</p>', mp_page)
  if not constituency_mo:
