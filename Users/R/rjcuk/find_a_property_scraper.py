...
DOMAIN = 'http://www.findaproperty.com/' 
...
def scrape_individual_house(house_url, saleType):
    ...
    # Record name
    house['title'] = house_tree.xpath('string(//h1[@id="Content_H1"])')
    house['title'] = house['title'].split("in")[0]

    # Record as Sales/Lettings
    house['saleType'] = saleType

    # Image
    house['image_url'] = house_tree.xpath('string(//img[@id="Content_DefaultPhotoImage"]/@src)')
    #print "Image:",house['image_url']
    
    # Get price into nice numerical-only format
    price = house_tree.xpath('string(//div[@class="propPrice"]/span[@class="highlight"])')
    price = price[2:]
    ...
    # Address
    address = house_tree.xpath('string(//title)')
    address = address.split("sale, ")[1]
    address = address.split(" - Ref:")[0]
    # strip out \t, \r, \n
...
def scrape_results_page(results_url, saleType, initial=False, domainPrefix=True):
    global iProp
    if domainPrefix:
    ...
    house_links = tree.xpath('//div[@class="picContainer"]/a/@href')
    ...
    if initial: # if the first page, do the rest of the pages!
        results_links = tree.xpath('//span[@class="pageNumbers"]//a/@href')
        for r in results_links:
            scrape_results_page(r, saleType, domainPrefix=False)
...
# 2. Sales
url1 = 'searchresults.aspx?edid=00&salerent=0&agentid=38954'
INITIAL_URL = url1
