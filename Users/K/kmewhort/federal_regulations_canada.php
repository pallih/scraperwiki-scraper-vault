...
$index_url = $base_url . '/eng/regulations/%s.html'; // %s = index (A-Z);
$start_index = 'P';
...
foreach(array_merge(range($start_index,'Z'), array('Num')) as $a){
    ...
    foreach($dom->find(".titlesList li") as $row){
      $r = array('jurisdiction' => 'federal', 'type' => 'regulation');
      ...
      $link = $base_url . '/eng/regulations/' . $row->find(".TocTitle",0)->href;
