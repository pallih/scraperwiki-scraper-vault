...
def scrapeRosterPage(theHtml,team_id,team_name):
...
def scrapeTeamList(theHtml):

    soup1 = BeautifulSoup(theHtml)
    
    option_list = soup1.find('select', {'id':'ps_team'})
    

    # loop through every team in the option list
    for option in option_list.findAll('option'):
    
        team_url_source = option['value']
        team_name = option.text

        if team_url_source != '' :
            team_id = team_url_source.split('=')[1]
            ...
            scrapeRosterPage(html2,team_id,team_name)
...
html1 = scraperwiki.scrape("http://mlb.mlb.com/mlb/players/?tcid=nav_mlb_players")
scrapeTeamList(html1)
