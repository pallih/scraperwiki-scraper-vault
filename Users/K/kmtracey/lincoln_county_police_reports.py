...
starting_page = 'http://www.lincolnsheriff.org/p2c/Summary_Disclaimer.aspx'
...
def parse_page_count(html):
    ...
    page_count = soup.find('span', attrs={'id': 'mainContent_lblPageCount'})
    if not page_count:
        print html
...
def get_pages(day):
    ...
    br['MasterPage$mainContent$txtDateFrom2'] = day
    br['MasterPage$mainContent$txtDateTo2'] = day
    br['__EVENTTARGET'] = 'MasterPage$mainContent$cmdSubmit2'
    response = br.submit()
    pages = [response.read()]
    ...
    for p in range(2, parse_page_count(pages[0]) + 1):
        ...
        br.select_form(nr=0)
        br.form.set_all_readonly(False)
        br['__EVENTTARGET'] = 'MasterPage$mainContent$gvSummary'
