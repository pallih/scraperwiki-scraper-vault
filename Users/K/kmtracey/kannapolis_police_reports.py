...
starting_page = 'http://kannapolisp2c.com/p2c/Summary_Disclaimer.aspx'
...
def parse_page_count(html):
    ...
    page_count = soup.find('span', attrs={'id': 'ctl00_mainContent_lblPageCount'})
    if not page_count:
        print html
...
def parse_results(html):
    ...
    for r in soup.findAll('tr', attrs={'class': 'EventSearchGridRow'}):
        _, _, _, _, _, _, date, inc_type, details_raw, location, _ = r.findAll('td')
...
def get_pages(day):
    ...
    br['ctl00$mainContent$txtDateFrom$txtDatePicker'] = day
    br['ctl00$mainContent$txtDateTo$txtDatePicker'] = day
    response = br.submit()
    pages = [response.read()]
    ...
    for p in range(2, parse_page_count(pages[0]) + 1):
        ...
        br['__EVENTTARGET'] = 'ctl00$mainContent$gvSummary'
...
for x in range(START_DAYS_AGO, START_DAYS_AGO + NUM_DAYS):
    ...
    scraperwiki.sqlite.save(['date', 'location'], results)import scraperwiki    
import re
import mechanize
import datetime
from BeautifulSoup import BeautifulSoup
