...
STARTING_URL = 'https://eservices.darebin.vic.gov.au/ePathway/Production/Web/GeneralEnquiry/EnquiryLists.aspx?ModuleCode=LAP'
...
COMMENT_URL_PREFIX = 'mailto:planning@darebin.vic.gov.au?subject='
...
def get_da_summary(page, da_container)
  ...
  record = {
   ...
   'address' => clean_whitespace(tds[3].at('span').inner_text + ", NSW"),
   'description' => CGI::unescapeHTML(clean_whitespace(tds[4].at('span').inner_text)),
...
p "IIIIIIIIII"
page_form_first = page.forms.first
page_form_first.radiobuttons[0].click
page = page_form_first.submit(page_form_first.button_with(:value => "Next"))

formatted_number = 1

while (formatted_number < 3000) do
  page.forms.first.field_with(:id => "ctl00_MainBodyContent_mGeneralEnquirySearchControl_mTabControl_ctl04_mFormattedNumberTextBox").value = formatted_number
  ...
  page = page.forms.first.submit(page.forms.first.button_with(:value => "New Search"))
end


page = @agent.get(STARTING_URL)
page_form_first = page.forms.first
page_form_first.radiobuttons[2].click
page = page_form_first.submit(page_form_first.button_with(:value => "Next"))

formatted_number = 5000

while (formatted_number < 50000) do
  page.forms.first.field_with(:id => "ctl00_MainBodyContent_mGeneralEnquirySearchControl_mTabControl_ctl04_mFormattedNumberTextBox").value = formatted_number
  page = page.forms.first.submit(page.forms.first.button_with(:value => "Search"))
  @index = 1
  @url = page.uri
