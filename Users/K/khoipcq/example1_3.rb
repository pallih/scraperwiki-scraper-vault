...
STARTING_URL = 'https://epathway-web.gleneira.vic.gov.au/ePathway/Production/Web/GeneralEnquiry/EnquiryLists.aspx'
...
COMMENT_URL_PREFIX = 'mailto:mail@gleneira.vic.gov.au?subject='
...
def get_da_summary(page, da_container)
  ...
  record = {
    ...
    'date_received' => Date.strptime(clean_whitespace(tds[3].at('span').inner_text), '%d/%m/%Y').to_s,
    'address' => clean_whitespace(tds[2].at('span').inner_text),
    'description' => CGI::unescapeHTML(clean_whitespace(tds[1].at('div').inner_text)),
