...
STARTING_URL = 'http://ecouncil.melton.vic.gov.au/eservice/daEnquiryInit.do?nodeNum=11139'
SEARCH_URL = 'http://ecouncil.melton.vic.gov.au/eservice/daEnquiry.do?number='
CHILD_URL = 'http://ecouncil.melton.vic.gov.au'
...
COMMENT_URL_PREFIX = 'mailto:melton@melton.vic.gov.au?subject='
...
def get_da_summary(page, da_container)
  ...
  record = {
   ...
   'address' => clean_whitespace(result["Property Details"].to_s),
   'description' => CGI::unescapeHTML(clean_whitespace(result["Proposal"].to_s)),
...
while (from_date.year > 1990) do
  ...
  item_count = page.search("div#fullcontent h4[class$=non_table_headers]").length
  @url = page.uri
  @index = 0
