...
BASE_URL = "http://www.aberdeencity.gov.uk/xsd_SOTD_Application.asp"
...
def scrape_table(page_body, page_num)
  ...
  data_table = Nokogiri::HTML(page_body).css('#ScoresTable tr').collect do |row|
    if row.css('td').count > 5
      ...
      business['Outcome']       = row.css('td')[4].inner_text
      business['Date Visted']   = row.css('td')[5].inner_text
...
for i in 2..@page_count
  page.form_with(:action => 'xsd_SOTD_Application.asp') do |f|
    f['page'] = i
    page = f.submit()
  end
