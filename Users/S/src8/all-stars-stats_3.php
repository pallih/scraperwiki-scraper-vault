...
if(preg_match('/(\d+),{0,1}(\d*).*/', $buckets, $matches)){ 
 $buckets=$matches[1] . $matches[2];
        ...
        foreach($locdom->find('a[class=dribbble-over] em') as $firstshotdate){}
            ...
            print $data->plaintext . '; ' . 'http://dribbble.com' . $data->href . '; ' . $loc->plaintext . ";" . $firstshotdate->plaintext . ";" . $followers . ";" . $following . ";" . $draftees . ";" . $shots . ";" . $projects . ";" . $likes . ";" . $tags . ";" . $buckets . "\n";

            scraperwiki::save(array('http'), array('http' => 'http://dribbble.com' . $data->href, 'name' => $data->plaintext, 'location' => $loc->plaintext, 'page' => $page . "", 'first shot' => $firstshotdate->plaintext, 'followers' => $followers, 'following' => $following, 'draftees' => $draftees, 'shots' => $shots , 'projects' => $projects , 'likes' => $likes, 'tags' => $tags, 'buckets' => $buckets));
