...
function first_page($cookie, $viewstate, $event_validation, $from, $to) {
    ...
    curl_setopt_array($ch, array(
        CURLOPT_URL => 'http://judis.nic.in/judis_gauhati/causetitle.aspx',
...
function not_first_page($cookie, $viewstate, $event_validation, $this_page_code) {
    $ch = curl_init();
    curl_setopt_array($ch, array(
        CURLOPT_URL => 'http://judis.nic.in/judis_gauhati/Chrseq.aspx',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_COOKIE => $cookie,
        CURLOPT_FOLLOWLOCATION => true,
        CURLOPT_POST => true,
...
for ($i = 6; $i > -1; $i = $i - 1) { //Count from XXX months ago
    $from = array(
        'day' => '01',
