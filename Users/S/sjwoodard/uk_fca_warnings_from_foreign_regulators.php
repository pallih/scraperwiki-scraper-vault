...
function scrape($page) {
    ...
    $dom = new DOMDocument();
    @$dom->loadHTML(
        scraperwiki::scrape("http://www.fca.org.uk/static/alerts/foreign/$page.html")
    );
    ...
    for ($i = 0; $i < $n; $i++) {
        ...
        array_push($results, @$result = array(
            ...
            'url' => $url,
            'source' => trim($row->childNodes->item(2)->nodeValue),
            'date' => trim($row->childNodes->item(4)->nodeValue)
        ));
