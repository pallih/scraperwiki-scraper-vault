...
function first_page($cookie, $viewstate, $event_validation, $from, $to) {
    ...
    curl_setopt_array($ch, array(
        CURLOPT_URL => 'http://judis.nic.in/judis_andhra/causetitle.aspx',
...
function not_first_page($cookie, $viewstate, $event_validation, $page) {
    $ch = curl_init();
    curl_setopt_array($ch, array(
        CURLOPT_URL => 'http://judis.nic.in/judis_andhra/Chrseq.aspx',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_COOKIE => $cookie,
        CURLOPT_FOLLOWLOCATION => true,
        CURLOPT_POST => true,
        CURLOPT_POSTFIELDS => http_build_query(array(
            '__VIEWSTATE' => $viewstate,
            '__EVENTVALIDATION' => $event_validation,
            '__EVENTTARGET' => 'DataGrid1$ctl24$ctl'. sprintf("%02s", $page),
            '__EVENTARGUMENT' => '',
        )),
    ));
    $data = curl_exec($ch);
...
function scrape($cookie, $viewstate, $event_validation, $page, $from, $to) {
    ...
    for ($i = 1; $i < $n; $i++) {
        ...
        @$result = array(
            ...
            'unique_id' => preg_replace('/\s+|\W+/', '', $row->childNodes->item(0)->nodeValue .'-'. $row->childNodes->item(1)->nodeValue .'-'. $from['day'].$from['month'].$from['year'] .'-'. $to['day'].$to['month'].$to['year'])
        );
    ...
    if ($page+1 < ($records_found/20)) {
        scrape($cookie, $viewstate, $event_validation, $page+1, $from, $to);
    }
}
...
for ($i = 4; $i > -1; $i = $i - 1) {  //Count from XX months ago
    $from = array(
    ...
    scrape($cookie, '/'.$aspnet_vars[1][0], '/'.$aspnet_vars[1][1], 0, $from, $to);
}
?><?php
//ASP.Net gives unique credentials when first running a query. Therefore, the script needed to acces the first page
//is more intensive than the script for all other pages. 
