...
items = re.compile('<tbody.*?class="adr-list">(.+?)</tbody>', re.I | re.U | re.S )
...
url="http://www.vtb.ru/group/contacts/geography/russia/angarsk/"
    ...
    if len(m1) == 0:
        ...
        if len(m1)>1:
            ...
            for t in m1:
                i+=1
                scraperwiki.sqlite.save(unique_keys=['id'], data={'id': i, 'url': url, 'lat': '', 'lon': '', \
                    'name': '', 'address': t, 'detail_page_url': ''})
