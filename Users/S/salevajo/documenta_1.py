...
url = re.search('URL=(.*)/form/find-simple"', sessionPage).group(1)
# Suchbegriff
search = "documenta+1"

for a in range(500):
    b = a + 1
    unterseite = url + "/sysix?SCAN_CODE=ALL%3AART&SCAN_START=" + str(1 + (b-1)*10) #klappert die Suchergebnisseiten ab, auf denen jeweils zehn Links zu Künstlern der gesuchten documenta stehen - wieviele Unterseiten es gibt, ist egal, Hauptsache, die Schleife ist lang genug, um alle Unterseiten zu erfassen (der Rest der Schleife produziert einfach keine Daten). Achtung, die Seite produziert nach einiger Zeit einen Session-Timeout, dann auf der Datenbankseite neu suchen und den Link hier ersetzen!
