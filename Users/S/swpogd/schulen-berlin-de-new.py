...
def scrape_tables_from_each_year_in_subpage(baseurl, urlparams, tablerow_css, tablerow_datacols, school):
    ...
    for year in year_list:
        root = scrape_and_assert_school(baseurl+"?"+urlparams+"&jahr="+year.text_content(), school)
...
#    scraperwiki.sqlite.execute("CREATE TABLE jahrgangsstufe (IDSchulzweig integer, SchulNr text, Schulzweig text, Jahr text, Jahrgangsstufe text, W integer, M integer)")
...
#foo = scraperwiki.sqlite.select("* from personal order by IDSchulzweig")
#scraperwiki.sqlite.execute("DROP TABLE IF EXISTS personal")
#scraperwiki.sqlite.execute("CREATE TABLE personal (IDSchulzweig integer, SchulNr text, Schulzweig text, Jahr text, " +
#            "Bezeichnung text, W_in_prozent integer, M_in_prozent integer, Insgesamt integer)")
#scraperwiki.sqlite.save(unique_keys=['IDSchulzweig', 'Jahr', 'Bezeichnung'], data=foo, table_name="personal")            
