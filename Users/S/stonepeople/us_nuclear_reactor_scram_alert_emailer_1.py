...
for line in scraperwiki.sqlite.select(" * from swdata order by date limit 50000 offset %s" % (x) ):
    ...
    if reason != None and '#' in reason:
        print 'On %s, %s reactor had power output of %s because %s' % (line[u'date'], line[u'Unit'], line[u'Power'], reason)

scraperwiki.sqlite.save_var('last_item', x )import scraperwiki
import sys
import time
