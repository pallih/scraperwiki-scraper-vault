...
$url = 'http://terpconnect.umd.edu/~parnia/iplant2/x1.output'; 
$contents = file_get_contents($url); 
    $convert = explode("\n", $contents); //create array separate by new line

for ($i=0;$i<count($convert);$i++)  
{
    $currentuser = $convert[$i];

//$html_content = scraperWiki::scrape("http://terpconnect.umd.edu/~parnia/iplant2/" . $currentuser);

//$html_content = scraperWiki::scrape("https://my-plant.org/users/all");
//require 'http://terpconnect.umd.edu/~parnia/simplehtmldom_1_5/simple_html_dom.php';
$gender = "";
$name = "";
//$html_content = file_get_contents('http://terpconnect.umd.edu/~parnia/iplant2/' . $currentuser);
//$html = str_get_html($html_content);

$html = file_get_html('http://terpconnect.umd.edu/~parnia/iplant2/' . $currentuser);
$allclades = array();
$colleague_array = array();
$wallposts = array();
$occupation = "";
$interests = "";
$about = "";
print "User " . $i . "\n";
foreach ($html->find("h1.nomargin") as $na) {
  print "Name: " . $na->plaintext . "\n";
$name = $na->plaintext;
}

foreach ($html->find("div.field-field-gender") as $ge) {

  //print trim($ge->plaintext) . "\n";
$gender = substr(trim($ge->plaintext), 14);
 print "Gender: " . $gender . "\n";
}

foreach ($html->find("div.field-field-occupation") as $occ) {
 
$occupation = substr(trim($occ->plaintext), 18);
 print "Occupation: " . $occupation . "\n";
}
print "Clades: ";
foreach ($html->find("span.field-content a") as $clade){

print $clade->plaintext . ", ";

array_push($allclades, $clade->plaintext);

}
print "\n";
//print_r($allclades);
foreach ($html->find("div#profile-about-me p") as $aboutme) {
  print "About me: " . $aboutme->plaintext . "\n";
$about = $aboutme->plaintext;
}

foreach ($html->find("div#profile-interests p") as $in) {
  print "Interests: " . $in->plaintext . "\n";
$interests = $in->plaintext;
}

print "Colleagues: ";
foreach ($html->find("div.reduce a") as $colleague) {
 print $colleague->plaintext . ",";
$a = $colleague->plaintext;

//print $a . "\n";
array_push($colleague_array, $colleague->plaintext);

}
print "\n";

print "Wall posts: ";
foreach ($html->find("td.views-field") as $wpost) {

$b = trim($wpost->plaintext);
if (strcmp($b, '') != 0 && strcmp($b, 'Acanthaceae') != 0 && strcmp($b, 'Cyperaceae') != 0){
print $b . ",";
array_push($wallposts, $b);
}
}
print "\n";
