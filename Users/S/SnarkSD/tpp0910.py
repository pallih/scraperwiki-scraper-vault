...
gid = j = 0
season = "2009-2010s"
gidarray = [20799,    20904,    20979,    20999,    21016,    21058,    21157,    21189]
...
# Get PPTime
    ...
    for td in tds: # iterate over each of the <td> elements (i.e. elements of the list called "tds")
        #print i, td.text
        try:
            if 'POWER PLAYS (Goals-Occurrences / Time)' in td.text or 'POWER PLAYS (Goals-Occurrences/Time)' in td.text or 'POWER PLAYS (Goals-Opp. / Power Play Time)' in td.text or 'POWER PLAYS (Goals-Opp./Power Play Time)' in td.text:
                PPT[3] = tds[i+7].text
                PPT[4] = tds[i+8].text
                PPT[5] = tds[i+9].text
                PPT[0] = tds[i+16].text
