...
gid = j = 0
season = "2012-2013s"
gidarray = [30171, 30172, 30173, 30174]
PPT = [0,0]
...
for gid in gidarray:
    ...
    for td in tds: # iterate over each of the <td> elements (i.e. elements of the list called "tds")
        if ((tds[i-3].text != "5") and td.text =="GOAL") and (tds[i-2].text == "PP"):
            try:
                ...
                pass
            if ('G' in awayskate or 'G' in homeskate) and tds[i + 1].text.startswith(hometeam):
                TeamHG += 1
    ...
    for td in tds: # iterate over each of the <td> elements (i.e. elements of the list called "tds")
        #print i, td.text
        if td.text == "Power Plays (Goals-Opp./PPTime)" or td.text == "AV. NUM. (B-Opp-Temps) / Power Plays (Goals-Opp./PPTime)":
            PPT[m] = tds[i+2].text
            m -= 1
        i += 1
    save = [hometeam, awayteam, season, gid, TeamHG, TeamHS, TeamHF, TeamHC, TeamAG, TeamAS, TeamAF, TeamAC, PPT]
    print save
