...
def scrape_ds_page(url):
    '''Scrape data from a dataset page.'''
    ...
    for rs in rowsscheda:
        ...
        if (fieldscheda == 'field field-name-field-ente-gestore field-type-text field-label-above'):
            manager = rs.findAll('div')[2].contents[0]
...
np = str(s[0]).split('"')[1].split('?')[1].split('=')[1]
