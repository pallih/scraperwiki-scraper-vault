...
$max_single_view_pages = 20000;
$max_list_view_pages = 20000;

foreach(range('I','Z') as $c) {
    if ($max_single_view_pages < 1) break;
    if ($max_list_view_pages < 1) break;

    echo "Orte mit \"" . $c . "\"..." . "\n";
    $params = "module=gepris&task=browseOrtsindex&character=" . $c . "&index=0";
    ...
    do {
        ...
        $locations = $xpath->query('//ul[contains(@class,"locationList")]/li/div[contains(@class,"location")]');
        echo "processing " . ($locations->length) . " locations...\n";
        
        foreach ($locations as $div_location) {
            if ($max_single_view_pages < 1) break;
            $title = $xpath->query($div_location->getNodePath() . '/h3');
            if ($title->length > 0) $title = $title->item(0);
            else die("exiting, xpath was " . $div_location->getNodePath() . '/h3');
            echo strip_off_multiple_spaces(strip_tags($title->nodeValue)) . "\n";
            $rows = $xpath->query($div_location->getNodePath() . '//tr/td/a');
            
            echo "processing " . ($rows->length) . " institutions...\n";
            foreach ($rows as $a) {
                if ($max_single_view_pages-- < 1) break;
