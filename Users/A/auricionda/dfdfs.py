...
while b !=5: 
    ...
    for el in root.cssselect("p.intro"):
        about=el.text
    f=html.count("dd")
    for el in root.cssselect("div.col dd")[g]:            
        lugar=el.tail
    g=g+1
    if(g==f-1):
        dos=''
    ...
    if(g==f-1):
        ...
        for el in root.cssselect("div.col dd")[g]:
            tres=el.text_content()
    g=g+1
    if(g==f-1):
        cuatro=''
    else:
        for el in root.cssselect("div.col dd")[g]:
            cuatro=el.text_content()
    g=g+1
    if(g==f-1):
        cinco=''
    else:
        for el in root.cssselect("div.col dd")[g]:
            cinco==el.text_content()
    g=g+1
    if(g==f-1):
        seis=''
    else:
        for el in root.cssselect("div.col dd")[g]:
            seis==el.text_content()
    scraperwiki.sqlite.save(unique_keys=['orden'],data={'orden':b,'about':about,'lugar':lugar,'id':str(a[b]),'dos':dos,'tres':tres,'cuatro':cuatro,'cinco':cinco,'seis':seis})
    b=b+1
    about=str('')
    g=0
