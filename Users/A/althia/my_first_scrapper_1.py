...
def our_scrapper(our_url):
    the_page = scraperwiki.scrape(our_url)
    for every_post in re.finditer('<p><a href="http://ottawa.en.craigslist.ca/mis/(.+?)</a>', the_page):  

        every_post = every_post.group(1)
        print every_post

    scraperwiki.sqlite.save(unique_keys=["listing"], data={"listing": the_page})
