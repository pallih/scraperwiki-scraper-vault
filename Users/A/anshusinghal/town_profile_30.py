...
uri="http://www.censusindia.gov.in/Census_Data_2001/Census_Data_Online/Area_Profile/SubDistrict_Profile.aspx?cki=haFRMqnZVKa"
...
districtcode=[]
district=[]
form.set_value(['09'], name="drpState")
        ...
        for k in towncode:
            try:
                if(k==""):
                    ...
                    #print "enter"
                    form.set_value([k],name="drpTeshil")
                    content=urlopen(form.click())
                    ...
                    for l in response.cssselect("tr.GridRows td"):
                        ...
                        if l_c<3:
                            ...
                            data.append(row)
                            print data
                            scraperwiki.sqlite.save(unique_keys=["S_no"],data={"S_no":s_no,"Column1":row[0],"Column2":row[1],"Column3":row[2],"Column4":row[3],"State":'Goa',"district":district[districtstore],"town":town[townstore]})
                            s_no+=2
                            row=[]
                    s_no=serial+1
                    for l in response.cssselect("tr.GridAlternativeRows td"):
                        if l_c<3:
                            ...
                            data.append(row)
                            #print row
                            scraperwiki.sqlite.save(unique_keys=["S_no"],data={"S_no":s_no,"Column1":row[0],"Column2":row[1],"Column3":row[2],"Column4":row[3],"State":'West Bengal',"district":district[districtstore],"town":town[townstore]})
                            s_no+=2
                            #print "saved"
                            row=[]
                    townstore+=1
