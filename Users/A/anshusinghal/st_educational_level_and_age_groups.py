...
uri ="http://www.censusindia.gov.in/Census_Data_2001/Census_Data_Online/Social_and_cultural/ST_Educational_level_and_Age_groups.aspx"
...
while state_count<35:
    ...
    for i in response.cssselect("tr.GridRows td"):
        if l_c<12:
            ...
            scraperwiki.sqlite.save(unique_keys=["S_no"],data={"S_no":s_no,"State":item.attrs,"Age_Groups":row[1],"Total_Population":row[2],"Literate_without_educational_level":row[3],"Below_primary":row[4],"Primary":row[5],"Middle":row[6],"Matric_secondary":row[7],"Higher_secondary":row[8],"Non-technical_diploma":row[9],"Technical_diploma":row[10],"Graduate_and_above":row[11],"Unclassified":row[12]})
            s_no+=2
            row=[]
