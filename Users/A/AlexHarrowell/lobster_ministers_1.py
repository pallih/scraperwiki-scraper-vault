...
def mini(mgraph):
    ministers = [node for node in mgraph.nodes(data=True) if node[1]['nodetype'] == 'minister']
    return ministers
...
def meeting_count(mgraph, month, node):
    ...
    meeting_count = {'Minister': node[0], 'Title': node[1]['Title'], 'Department': node[1]['Department'], 'Date': month, 'Meetings': mc}
    return meeting_count

def network_degree(mgraph, month, node):
    degrees = {'Minister': node[0], 'Title': node[1]['Title'], 'Department': node[1]['Department'], 'Date': month, 'Network Degree': mgraph.degree(node[0], weight='weight')} 
    return degrees

def average_minister_weighting(mini, average_weight):
    if not average_weight:
        miniweights = [node[1]['minister_weight'] for node in mini]
        average_weight = float(sum(miniweights))/float(len(miniweights))
...
def gatekeepers(mgraph, month, node, average_weight):
    av_weight = average_weight
    # calculate average weight of a minister
    def gk(node, mgraph):
            mweight = node[1]['minister_weight']
            minister_factor = mweight/av_weight
            neighbours = mgraph.neighbors(node[0])
            edges = [e[2]['weight'] for e in mgraph.edges(neighbours, data=True)]
            average_edge_weight = float(sum(edges))/float(len(edges))
            return (average_edge_weight/av_weight)/minister_factor
    gatekeepership = {'Minister': node[0], 'Title': node[1]['Title'], 'Department': node[1]['Department'], 'Date': month, 'Gatekeepership': (gk(node, mgraph))}
    return gatekeepership

def centrality(mgraph, month, mini, nodes):
    central = [{'Minister': k, 'Title': mgraph.node[k]['Title'], 'Department': mgraph.node[k]['Department'], 'Date': month, 'Centrality': v} for k,v in nodes.iteritems() if k == mini[0]]
    return central[0]
...
def greedy_fragile(mgraph, month, mini, nwc, order, nodes):
    ...
    return {'Minister': mini[0], 'Title': mini[1]['Title'], 'Department': mini[1]['Department'], 'Date': month, 'Greedy_Fragile': gf}
...
def control_loop(lms, rankings, average_weight, nodes):
    ...
    for month in calendar:
        if time_munger(month) >= time_munger(lms):
            ...
            ministers = mini(graph)
            average_weight = average_minister_weighting(ministers, average_weight)
            ...
            for m in ministers:
                ...
                op.update(gatekeepers(graph, month['Date'], m, average_weight))
                ...
                output.append(op)
            scraperwiki.sqlite.save(unique_keys=['Minister', 'Department', 'Title', 'Date'], data=output, verbose=0)
