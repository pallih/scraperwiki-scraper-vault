#TODO add tides from http://live.getij.nl/export.cfm?format=txt&from=14-10-2012&to=14-10-2012&uitvoer=1&interval=30&location=SCHEVNGN&Timezone=MET_DST&refPlane=NAP&graphRefPlane=NAP&bottom=0&keel=0
#TODO add sunrise and sundown times
    ...
    if len(tdsw)==5:
        ...
        if j:
            ...
            for k in range(6): #6 half hours in 3h
                #Let's read line j of, and focus on the tide in cm.
                ...
                if dataWaves['datetime'].time() > sunTimes['sunrise'] and dataWaves['datetime'].time() < sunTimes['sunset']:
                    dayLight=1
                else:
                    dayLight=0
                ...
                dataCombined = {
                    'Bft':prevData['Bft']+int((data['Bft']-prevData['Bft'])/6*k),
                    'datetime': prevData['datetime']+datetime.timedelta(0,0,0,0,30*k), #increment in half hours
