...
def scrape_table(soup):
    data_table = soup.find("form", { "id" : "default.aspx?c=1080" })
...
def scrape_and_look_for_next_link(soup):
    scrape_table(soup)
    next_link = soup.find("a", { "id" : "Form1" })
    if next_link:
...
base_url =  'http://www.dfm.ae'

#'http://recognition.ncqa.org/'
starting_url = base_url + r'/pages/default.aspx?c=1080' #+ 'PSearchResults.aspx?state=NY&rp=
