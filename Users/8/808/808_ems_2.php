...
foreach($sources as $page) {
    ...
    //determine if there are page errors
    //foreach($dom->find("<div[@class='bodya_warn'] tr") as $data0){  
    //foreach($dom->find("'a'class='topLink'") as $data0){
        //if ($data0->plaintext='next'){
    //foreach($dom->find('a[@>"Next"]',0) as $data0){
        //$pageErrorCount++;
        //$GotResultsCount ++;

        //if ($pageErrorCount == 0) {        
        //if($next = $dom->find('a[class=topLink @>"Next"]')) {
        //if ($pageErrorCount > 0) {
        //if ($gotResults > 0){
        ...
        //find the table of persons and iterate through each TR
            ...
            if(count($tds)==2){
                ...
                //scraperwiki.sqlite.save(unique_keys=['name'], data=array());
