...
$startseite = scraperwiki::get_var('last_page')+1; //+1

$offset = 15;
$durchgaenge = 10;

print "Starte ab Seite: ".$startseite."\n";

for ($seite=$startseite; $seite<=$startseite+$durchgaenge-1; $seite++) {

    print "Bearbeite Seite: ".$seite."\n";

    $html_content_s = scraperwiki::scrape("http://www.mydirtyhobby.com/?ac=search&ac2=umgebung&plz=2#type|1|ac|search|anf|".$seite*$offset."|ac2|umgebung|plz|2|gender|F|module|get");
    ...
    foreach ($html_s->find("div.amateurs_block .am_name_block a") as $el_s) {
        ...
        foreach ($html->find("#prof_bildl1 img") as $profilBild) {
            ...
            if (empty($profilBild)) {
                foreach ($html->find("#prof_bildl2 img") as $profilBild) {
                    $profilBild = str_replace('<img id="prof_bild2" src="',NULL,$profilBild);
                    $profilBild = str_replace('" alt="" border="0">',NULL,$profilBild);
                }
            }
    ...
    scraperwiki::save_var('last_page', $seite);
}
?><?php
require 'scraperwiki/simple_html_dom.php';
