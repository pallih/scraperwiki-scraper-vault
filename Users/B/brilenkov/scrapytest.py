...
class SpeakersSpider(BaseSpider):
    ...
    def parse(self, response):
        ...
        for sel in hxs.select('//*[@id="speakers_list"]/li'):
            il = SpeakerLoader(selector=sel)
            il.add_xpath('name', './/div[@class="speakers-box-right"]/h2/text()')
            il.add_xpath('image', './/div[@class="speakers-box clearfix"]/img/@src')
            il.add_xpath('description', './/div[@class="speakers-box-right"]/p/*')
            yield il.load_item()
