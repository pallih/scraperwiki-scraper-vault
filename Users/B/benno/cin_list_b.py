...
def writeData():
    scraperwiki.sqlite.execute("drop table if exists cinema_list_berlin")
    scraperwiki.sqlite.execute("create table cinema_list_berlin ( `kino` text, `adresse` text, `plz` text, `ort` text, `telefon` text, `uid` text, `url` text)")
    ...
    for i in range(len(result)):
        ...
        scraperwiki.sqlite.save(unique_keys=["kino", "adresse", "plz", "ort", "telefon", "url", "uid"], data=data, table_name="cinema_list_berlin", verbose=7)

def main():
    html = scraperwiki.scrape("http://www.berlinien.de/kino/kinos_region_berlin.html")
