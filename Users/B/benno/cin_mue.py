...
html = scraperwiki.scrape("http://www.munig.com/kino/kinoprogramm.html") #  die quelle, eine tabelle wird zeilenweise gescrapt..
...
def createEvent(element):                             #     Datum, Kino_name, Filme, (Film_Url), Zeiten
    ...
    if  cinema_flag > 0:
        result.append(cinema)                         # a list of all the cinemas to generate a seperate csv for cinemas+adreses, toDo!
...
def writeData():
    tagesprogramm["filme"].append(movies)
    scraperwiki.sqlite.execute("drop table if exists cin_mun")
    scraperwiki.sqlite.execute("create table cin_mun ( `kino` text, `film` text, `zeit` text, `add_info` text, `datum` text)")
    scraperwiki.sqlite.commit()
    print "Anzahl Filme: ", len(tagesprogramm["filme"])
    for i in range(len(tagesprogramm["filme"])):
        for j in range(len(tagesprogramm["filme"][i])):                        # daten umsortieren und in die Db!   kino,filmtitel,uhrzeit,datum
            ...
            scraperwiki.sqlite.save(unique_keys=["kino", "datum", "zeit", "add_info", "film"], data=kinoprogramm, table_name="cin_mun", verbose=5)
