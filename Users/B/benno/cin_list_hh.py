...
def writeData():
    scraperwiki.sqlite.execute("drop table if exists cinema_list_hh")
    scraperwiki.sqlite.execute("create table cinema_list_hh ( `kino` text, `adresse` text, `plz` text, `ort` text, `telefon` text, `uid` text, `url` text)")
    ...
    for i in range(len(result)):
        ...
        scraperwiki.sqlite.save(unique_keys=["kino", "adresse", "plz", "ort", "telefon", "url", "uid"], data=data, table_name="cinema_list_hh", verbose=7)

def main():
    html = scraperwiki.scrape("http://hamburg.stadtus.de/kino/kinos_region_hamburg.html")
