...
class extractor(object):
    ...
    def extract_apikey(self):
        
        pattern = '\".*\"'
        
        string_html = lxml.html.tostring(self.article.tree)
        #html = scraperwiki.scrape(detailsPageAddr)

        split_html = string_html.split()
        print string_html

        for line in split_html:
            if "AppKey" in line:
                regexResults = re.search(pattern, line)
                return regexResults.group(0).strip('\"')
...
class navigator(object):
    ...
    def gather_links(self, link_selector):
        ...
        for link in link_list:
            link_string = lxml.html.tostring(link)
            if link_string not in link_strings:
    ...
    def links_to_articles_two(self):
        i = 0
        for link in self.links:
            self.article.change_page(link)
            linked_article = web_article(self.article.get_url())
            if self.article.get_tree() is not None:
                self.linked_articles.append(linked_article)
                i += 1
...
def extract_data():
    # Set root url
    url = 'http://b3.caspio.com/dp.asp?AppKey=136b1000a954333708654344941d'
    # Make root page
    root_page = web_article(url)
    print lxml.html.tostring(root_page.tree)
    # Create navigator for getting data
    root_navigator = navigator(root_page)
    # Gather all detail pages
    root_navigator.gather_linked_articles('td[class="cbResultSetTableCell cbResultSetData"] a')
    # For each detail app
    for detail_app in root_navigator.linked_articles:
        print detail_app.url
        print lxml.html.tostring(detail_app.tree)
        apikey_extractor = extractor(detail_app)
        apikey = apikey_extractor.extract_apikey()
        print apikey
        app_navigator = navigator(detail_app)
        app_navigator.links = [apikey]
        app_navigator.links_to_articles_two()
        detail_article = app_navigator.linked_articles[0]
        print detail_article.url
...
extract_data()import scraperwiki
