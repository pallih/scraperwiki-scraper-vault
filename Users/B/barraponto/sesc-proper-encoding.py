...
class SESCProgram:
    def __init__(self, tree, page):
        self.program = tree.cssselect('.tit')[0].text_content().rstrip(tree.cssselect('.tit font')[0].text_content())
        self.program_id = re.search(r'[0-9]+', tree.cssselect('.tit')[0].get('onclick')).group(),
        self.events = []
        ...
        for k,title in enumerate(tree.cssselect('.tit2')):
            self.events.append({
                ...
                'date_string': re.sub('\s+', ' ', tree.cssselect('.data tr')[2 * k].text_content()),
...
class SESCEvent:
    ...
    def __init__(self, tree, page):
        self.title = tree.cssselect('.tit')[0].text_content()
        self.date_string = re.sub('\s+', ' ', tree.cssselect('.data td')[0].text_content())
        ...
        precos = [cell.text_content().strip() for cell in self.tree.cssselect('.preco')]
