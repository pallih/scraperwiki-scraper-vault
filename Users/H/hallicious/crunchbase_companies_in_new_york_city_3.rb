...
class Crunch
  def initialize(page)
    @crunch = Nokogiri::HTML(ScraperWiki.scrape("http://www.crunchbase.com/search?query=health"))
  end
  
  def parse
    @crunch.css(".search_result_name a").each do |company_link|
      crunch_link = "http://www.crunchbase.com" + company_link.attr("href")
        company = {'name' => escape_csv(company_link.content), 'url' => crunch_link, 'employees' => company_people(crunch_link), 'funding' => company_fundings_round_total(crunch_link)}
      ScraperWiki.save(['url', 'funding', 'name', 'employees'], company)
    end
  end
  ...
  def company_people(company_link)
    ...
    company_page = Nokogiri::HTML(ScraperWiki.scrape(company_link))
        company_page.css(".col1_fundings_round_total").each do |funding|
      titles << escape_csv(funding.content)
    end
