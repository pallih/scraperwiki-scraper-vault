...
STARTING_URL = 'http://www.albany.wa.gov.au/website/page.php?folder=1322'
...
def get_da_summary(page, da_container)
  ...
  info_url = da_container[0].search('td')[2].at('a')['href']
  application_number = da_container[0].search('td')[1].inner_text
  applicant   = da_container[1].search('td')[1].inner_text
  recice_date = da_container[2].search('td')[1].inner_text
  desc = da_container[3].search('td')[1].inner_text
 
  #tds = da_container.search('td')

  record = {
    'info_url' => page.uri + info_url,
    'council_reference' => clean_whitespace(application_number),
    'date_received' => Date.strptime(clean_whitespace(recice_date), '%d/%m/%Y').to_s,
    'description' => CGI::unescapeHTML(desc),
...
#button = page.search("input[value=Search]")
#p button.first

form = page.form_with(:id=>"syn_newDevelopmentApplicationSearchForm")
button = form .button_with(:id=>"syn_submitNewDevelopmentApplicationSearch")
page = form.click_button(button)
#p page.body

#page = page.forms.first.click_button(button)
#p page.body
