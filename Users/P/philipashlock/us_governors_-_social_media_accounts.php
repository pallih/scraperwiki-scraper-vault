...
$directory_url = "http://www.govsm.com/w/Governors";
...
function get_sources($url) {
    ...
    foreach($list->find("tr") as $row){
        ...
        if ($count > 2) {
              ...
              $record['state'] = trim($row->find("td", 4)->plaintext);                        
            
            
               $record['facebook'] = ($row->find("td", 5)->find('a',0)) ? $row->find("td", 5)->find("a", 0)->href : null;    
            $record['twitter'] = ($row->find("td", 6)->find('a',0)) ? $row->find("td", 6)->find('a',0)->href : null;    
            $record['youtube'] = ($row->find("td", 7)->find('a',0)) ? $row->find("td", 7)->find('a',0)->href : null;    
            $record['flickr'] = ($row->find("td", 9)->find('a',0)) ? $row->find("td", 9)->find('a',0)->href : null;                
