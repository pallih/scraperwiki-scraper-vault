...
def scrape(data)
    records = []
    doc = Nokogiri::HTML(data).xpath(".//table[@class='simpleGrid results selectable highlight']/tr")
    doc.each{|tr|
      td = tr.xpath("td")
      r = {
        "COMPANY_NAME" => text(td[0].xpath("div[@class='businessName']")),
        "URL" => BASE_URL + attributes(td[1].xpath("a"),"href"),
        "DOC" => Time.now
      }
      details = text(td[0].xpath("div[@class='details']")).flatten.first
      
      r["COMPANY_NUMBER"] = r["URL"].split("?")[0].split("/").last unless r["URL"].nil? 
      records << r unless details =~ /Trademark|Assumed Name/
    }
...
def action(srch)
  begin
    ...
    s_url = BASE_URL + "/Business/BusinessSearch?BusinessName=#{srch}&Status=Active&Type=BeginsWith"
    pg = br.get(s_url)
