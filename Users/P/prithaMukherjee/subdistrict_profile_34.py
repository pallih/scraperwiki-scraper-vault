...
districtcode=[]
districtname=[]
form.set_value(['34'], name="drpState")
            ...
            if(l==""):
                ...
                for k in response.cssselect("tr.GridRows td"):
                    if l_c<3:
                        ...
                        l_c=0
                        data.append(row)
                        scraperwiki.sqlite.save(unique_keys=["S_no"],data={"S_no":s_no,"Column1":row[0],"Column2":row[1],"Column3":row[2],"Column4":row[3],"State":'Puducherry',"district":districtname[districtcount],"Sub_district":sub_districtname[sub_districtcount]})
                        s_no+=2
                        row=[]
