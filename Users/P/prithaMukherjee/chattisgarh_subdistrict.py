...
districtcode=[]
districtname=[]
form.set_value(['22'], name="drpState")
...
for j in districtcode:
            ...
            if(l==""):
                ...
                for k in response.cssselect("tr.GridRows td"):
                    if l_c<3:
                        ...
                        l_c=0
                        data.append(row)
                        scraperwiki.sqlite.save(unique_keys=["S_no"],data={"S_no":s_no,"Column1":row[0],"Column2":row[1],"Column3":row[2],"Column4":row[3],"State":'Chattisgarh',"district":districtname[districtcount],"Sub_district":sub_districtname[sub_districtcount]})
                        s_no+=2
                        row=[]
    ...
    districtcount+=1
#    statecount+=1import scraperwiki
import lxml.html
from mechanize import ParseResponse, urlopen, urljoin
