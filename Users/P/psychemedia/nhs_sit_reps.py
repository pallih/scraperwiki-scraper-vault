...
def dateNormalise(d):
    ...
    for trials in ["%d %b %y","%d %m %y","%d %b%y",'%d-%b-%y','%d-%b-%Y','%d/%m/%Y','%d/%m/%y','%d %b %Y','%d-%m-%Y','%d/%m%Y', '%d-%b %y','%d-%b%y']:
        try:
            d=d.strip()
...
def dateRangeParse(daterange):
    ...
    #Horrible hack - need to work on this
    daterange=daterange.replace('2012 -','2012 to')

    daterange= re.sub(re.compile(r'\s+'),'',daterange)

    dd=daterange.split('to')

    if len(dd)<2:
        #14-16-Dec 12, 14-16 Dec 12, 14-16-Dec-12 but not eg 2012-12-11;
        #1-2-Jan13
        p=re.compile('(\d+)-(\d+[\s-]+[A-Za-z]+[\s-]*\d*)')
        m=p.match(daterange)
        if m!=None:
            fromdate=m.group(1)
            todate=m.group(2)
    ...
    for sheetname in book.sheet_names():
        ...
        for row in range(17, sheet.nrows):
            ...
            for col in range (4,sheetwidth):
                ...
                #HORRIBLE HACK - https://www.wp.dh.gov.uk/transparency/files/2012/10/DailySR-Web-file-WE-06-01-13.xls error
                if data['facetB']==41264.0:
                    data['facetB']="Urg ops canc'd 2 or more times"
