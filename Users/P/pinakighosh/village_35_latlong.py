...
import lxml.html
import sys
url="http://censusindia.gov.in/Census_Data_2001/Village_Directory/List_of_Villages/List_of_Villages_Alphabetical.aspx?cki=&State_Code=35"
import string
...
#save to data base
                     ...
                     name=row[1]+","+row[3]+","+row[4]+","
                     name=name+",India"
                     #place, (lat, lng) = g.geocode(name,exactly_one=False)  
                     #print "%s: %.15f, %.15f" % (place, lat, lng)  
                     
                     try:
                        for place, (lat, lng) in g.geocode(name,exactly_one=False):
                            p,la,lo= place, lat, lng  
                            print p
                            print la
                            print lo
                            scraperwiki.sqlite.save(unique_keys=["sl_no"], data={"sl_no":s_no,"village_name":row[1], "village_code":row[2],"Sub_district_Name":row[3],"District_Name":row[4],"latitude":la,"longitude":lo})
                            s_no+=1
                     except:
                        scraperwiki.sqlite.save(unique_keys=["sl_no"], data={"sl_no":s_no,"village_name":row[1], "village_code":row[2],"Sub_district_Name":row[3],"District_Name":row[4],"latitude":0,"longitude":0})
                        s_no+=1
                     time.sleep(2)
