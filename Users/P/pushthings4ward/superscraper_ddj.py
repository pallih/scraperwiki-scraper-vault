...
for (q,table) in QUERY:
    for page in range(1, NUM_PAGES+1):
        base_url = 'https://api.twitter.com/1.1/search.json?q=%s&rpp=%s&lang=%s&page=%s&result_type=recent' \
        ...
        try:
            ...
            for result in results_json['results']:
                ...
                data['hashtags']='::'.join(thashtags).lower()

                thashtags=re.findall("#([a-z0-9]+)", result['text'], re.I)
                data['hashtagscount']=len(thashtags)
                ...
                thashtags=re.findall("http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+", result['text'], re.I)
                data['linkscount']=len(thashtags)

                thashtags=re.findall(unicode("pioneer", 'utf-8'), result['text'], re.I)
                if len(thashtags) > 0:
                    data['spam']='false'
                else:
                    data['spam']='true'
