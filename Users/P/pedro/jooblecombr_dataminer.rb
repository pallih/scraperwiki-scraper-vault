...
# Jooble.com.br premium ads dataminer
...
while j<queries.count
  queryurl='http://jooble.com.br/search-empregos-br/kw-'+queries[j]
#  puts queryurl
  doc = Nokogiri::HTML(open(URI::encode(queryurl)))
# doc.xpath("//*[@*[contains(., 'session')]]").each do |ele|
#//*[contains(@id, 'f41_txt')]
  doc.search("//td[contains(@id,'tca')]").each do |node|
   # puts node
    innode=node.search("img")
      if(innode.count>0) 
      #  puts "found an image!"
        #puts innode.attr('alt')
        name=innode.attr('alt').inner_html
     # puts name
      #else 
      #  puts "no images"
      end
