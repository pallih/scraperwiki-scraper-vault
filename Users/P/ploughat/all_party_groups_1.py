...
class AllPartyGroupsScraper(object):
    ...
    def get_page(self, url):
        ...
        html = fix_exceptions_plain(html)
        html = fix_exceptions_party(html)
        foo = etree.HTML(html)
        return foo
    ...
    def scrape_page(self, group_name, url):
        ...
        foo=(
                ('title','//tr[1]/td[2]/p')
        ,       ('purpose','//tr[3]/td[2]/p')
        ,       ('contact','//tr[16]/td[1]/p')
        ,       ('benefits','//tr[18]/td[1]/p')
        ,       ('emplayment','//tr[20]/td[1]/p')
        )

        for key,xpath in foo:
            try:
                group[key] = remove_html_tags(etree.tostring(doc.xpath(xpath)[0])).strip()
            except:
                group[key] = ""
        ...
        for i, col in enumerate(doc.xpath('//tr[count(td)=5]/td')):
            if i == 1:
                ...
                for lord in tmp:
                    if ('-' or u'–' or u'\u2013') in lord:
                        for hyphen in ('-', u'–', u'\u2013'):
                            try:
                                name, party = lord.rsplit(hyphen, 1)
                            except:
                                pass
                ...
                assert(len(others) == len(others_parties))
