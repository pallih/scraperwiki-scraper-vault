...
STARTING_URL = 'http://www.southburnett.qld.gov.au/web/guest/public-notification-of-development-applications'
...
def get_all_das_in_page(page)
  ...
  # For example:
  containers = page.search('table tbody tr')
  containers = containers[1..containers.length]

  containers .each do |da_container|
...
def get_da_summary(page, da_container)
  ...
  record = {
    ...
    'comment_url' => COMMENT_URL_PREFIX + CGI::escape("Development Application Enquiry: " + clean_whitespace(h[0])),
    'council_reference' => clean_whitespace(h[0]),
    #'date_received' => Date.strptime(clean_whitespace(h[2]), '%d/%m/%Y').to_s,
    'address' => clean_whitespace(tds[5].inner_text),
    'description' => CGI::unescapeHTML(clean_whitespace(h[5])),
