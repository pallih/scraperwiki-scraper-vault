...
STARTING_URL = 'https://epathway.wollongong.nsw.gov.au/ePathway/Production/Web/GeneralEnquiry/EnquiryLists.aspx'
...
COMMENT_URL_PREFIX = 'mailto:eapplications@wollongong.nsw.gov.au?subject='
...
def get_da_summary(page, da_container)
  ...
  tr = detail_page.search('table.ContentPanel tr[class$=ContentPanel]')[2]
  tds_detail_address = tr.search('td')
  if !tds_detail_address[0].at('span').nil? 
    address = clean_whitespace(tds_detail_address[0].at('span').text)
  else
    address = clean_whitespace(tds_detail_address[0].text)
  end
  ...
  record = {
    ...
    'address' => address,
    'description' => CGI::unescapeHTML(clean_whitespace(tds[1].at('span').text)),
...
@index = 1
@url = "https://epathway.wollongong.nsw.gov.au/ePathway/Production/Web/GeneralEnquiry/EnquirySummaryView.aspx"
def get_next_url
  @index = @index + 1 
