...
STARTING_URL = 'http://jrpp.nsw.gov.au/DevelopmentRegister/tabid/62/language/en-AU/Default.aspx'
...
COMMENT_URL_PREFIX = 'mailto:jrppenquiry@jrpp.nsw.gov.au?subject='
...
def has_next_page(page)
  next_page_condition = page.search('input[id$=dnn_ctr424_Public_lbtnNext]')
  unless next_page_condition.nil? 
    return true
  else
    return false
  end
...
def get_all_das_in_page(page)
  ...
  # For example:
  trs = page.search('table[id$=dnn_ctr424_Public_GVSearch] tr[class$=Row]')
  p trs.length
  page.search('table[id$=dnn_ctr424_Public_GVSearch] tr[class$=Row]').each do |da_container|
    #p da_container
...
'ctl00%24cphContent%24ctl00%24ctl00%24cphContent%24ctl00%24Radajaxpanel2Panel%7Cctl00%24cphContent%24ctl00%24ctl00%24RadGrid1%24ctl00%24ctl03%24ctl01%24ctl10'
    ...
    form.click_button(form.button_with(:id => 'dnn_ctr424_Public_lbtnNext'))
  else
    p "nillll"
    return nil
  end
...
def get_da_summary(page, da_container)
  ...
  record = {
    #'info_url' => (page.uri + tds[3].at('a')['href']).to_s,
    'comment_url' => COMMENT_URL_PREFIX + CGI::escape("Development Application Enquiry: " + clean_whitespace(tds[3].at('a').text)),
    'council_reference' => clean_whitespace(tds[3].at('a').text),
    #'date_received' => Date.strptime(clean_whitespace(tds[5].at('a').text), '%d/%m/%Y').to_s,
    'address' => "",
    'description' => CGI::unescapeHTML(clean_whitespace(tds[1].at('a').text)),
...
page = page.forms.first.submit(page.forms.first.button_with(:id => "dnn_ctr424_Public_btnSearch"))
