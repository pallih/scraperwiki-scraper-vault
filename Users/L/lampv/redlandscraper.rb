...
##
#Class::RedlandScraper
#
class RedlandScraper < InfoMasterScraper
  def applications(date)
    base_path = "http://pdonline.redland.qld.gov.au/modules/applicationmaster/"
    ...
    raw_table_values(date, "#{base_url}?page=search", 1).map do |values|
      ...
      da = DevelopmentApplication.new(
        ...
        :description => extract_description(values[3]).gsub(/^Description: /,"")
      )
...
new_scraper = RedlandScraper.new("Redland City Council", "Redland", "QLD")
