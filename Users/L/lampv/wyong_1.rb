...
STARTING_URL = 'http://wsconline.wyong.nsw.gov.au/applicationtracking/modules/applicationmaster/default.aspx?page=search'
...
while (Date.parse(from_date.strftime('%d/%m/%Y').to_s) >= Date.parse('01/07/2007')) do
  p "test"
  p from_date.month
  if !page_form_search.field_with(:id => "_ctl2_drDates_txtDay1").nil? 
    page_form_search.field_with(:id => "_ctl2_drDates_txtDay1").value = from_date.day.to_s
    page_form_search.field_with(:id => "_ctl2_drDates_txtMonth1").value = from_date.month.to_s
    page_form_search.field_with(:id => "_ctl2_drDates_txtYear1").value = from_date.year.to_s
    
    page_form_search.field_with(:id => "_ctl2_drDates_txtDay2").value = current_date.day.to_s
    page_form_search.field_with(:id => "_ctl2_drDates_txtMonth2").value = current_date.month.to_s
    page_form_search.field_with(:id => "_ctl2_drDates_txtYear2").value = current_date.year.to_s
  
    page = page_form_search.click_button(page.forms.first.button_with(:id => "_ctl2_btnSearch"))
    get_all_das(page)  
