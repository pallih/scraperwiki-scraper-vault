#scraper is to scrape the summary of all Development Applications for a Sample Council
...
STARTING_URL = 'http://planning.nmc.tas.gov.au/modules/ApplicationMaster/default.aspx?page=found&1=1/01/2000&2=5/01/2013&3=&4=1&4a=1&6=F&11=&12=&13=&14=&21=&22=&23='
...
COMMENT_URL_PREFIX = 'mailto:planning@northmidlands.tas.gov.au?subject='
...
def get_da_summary(page, da_container)
  ...
  table = detail_page.search('table')[0]
  trs = table.search('tr')
  
  tr_des = trs[1]
  tds_des = tr_des.search('td')
  h_des = tds_des.map{|td| td.inner_html}
  description = h_des[1].split('<br>')[0].split(':')[1]

  tr_address = trs[5]
  tds_address = tr_address.search('td')
  #h_address = tds_address.map{|td| td.inner_html}
  if !tds_address[1].at('a').nil? 
    address = tds_address[1].at('a').text
  else
    address = tds_address[1].text
  end
  ...
  record = {
    ...
    'description' => CGI::unescapeHTML(clean_whitespace(description )),
...
page = page.forms.first.submit(page.forms.first.button_with(:id => "cphContent_ctl00_btnSearch"))
page = agent.get(STARTING_URL)
