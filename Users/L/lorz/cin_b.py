...
html = scraperwiki.scrape("http://www.berlinien.de/kino/kinoprogramm.html") # http://www.munig.com/kino/kinoprogramm.html die quelle, eine tabelle wird zeilenweise gescrapt..
...
def createEvent(element):                             #     Datum, Kino_name, Filme, (Film_Url), Zeiten
    ...
    for kino in element.cssselect("b a"):                      
        try:
        ...
        cinema["url"] = kino.attrib["href"].replace('http://www.berlinien.de', '')
    ...
    for zeit in element.cssselect("td.smalltxt"):                # filme und zeiten sind in der class"smalltxt", beim 1.mal filmname, beim2.mal zeit!
        for film in element.cssselect("a.link1"):                 
            try:
        ...
        if not element.cssselect("a.link1"):
            ...
            for z in range(len(zeiten)):
                if len(zeiten[z]) > 5:
                    ...
                    zeit = zeiten[z]
                    add_info = u""
                movies_clone.append({"filmname": filmname, "kino": result[index]["name"], "kino_url": result[index]["url"], "datum": result[index]["datum"], "zeit": zeit, "add_info": add_info})
            movies = movies_clone
...
def writeData():
    tagesprogramm["filme"].append(movies)
    scraperwiki.sqlite.execute("drop table if exists cin_berlin")
    scraperwiki.sqlite.execute("create table cin_berlin ( `kino` text, `kino_url` text, `film` text, `zeit` text, `add_info` text, `datum` text)")
    ...
    for i in range(len(tagesprogramm["filme"])):
        for j in range(len(tagesprogramm["filme"][i])):                        
            kino = tagesprogramm["filme"][i][j]["kino"]
            kino_url= tagesprogramm["filme"][i][j]["kino_url"]
            ...
            kinoprogramm = {"zeit": film_zeit, "add_info": film_add_info, "datum": film_datum, "kino": kino, "kino_url": kino_url, "film": film_titel}
            scraperwiki.sqlite.save(unique_keys=["kino", "datum", "zeit", "add_info", "film"], data=kinoprogramm, table_name="cin_berlin", verbose=5)
