...
for row in reader:
    if row['title'] and row['link']:
        ...
        phtml = phtml.strip()
        if phtml:
            scraperwiki.sqlite.save(unique_keys=[], data={"id":i, "p_text":phtml}, table_name="p_table")
        else:
            scraperwiki.sqlite.save(unique_keys=[], data={"id":i, "p_text":"blank"}, table_name="p_table")
    i += 1
import scraperwiki
import csv
import lxml.html
