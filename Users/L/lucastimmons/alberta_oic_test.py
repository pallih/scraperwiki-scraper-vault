...
import scraperwiki
html = scraperwiki.scrape('http://www.qp.alberta.ca/rss/rss_QP_OCS_ALL.xml')
...
# Probably should use a rss parser
import lxml.html
root = lxml.html.fromstring(html) # turn our HTML into an lxml object
tds = root.cssselect('item title') # get all the <title> nested in <item>
for item in tds:
...
for item in tds:
     record = { "item" : item.text } # column name and value
     scraperwiki.sqlite.save(["item"], record) # save the records one by one
