...
url_refurb_store = 'http://store.apple.com/ca/browse/home/specialdeals/mac/'
...
for product in products:
    ...
    for item in items:
        ...
        save        = item.find('p', {'class' : 'savings'})
        
        #print scrape_datetime, product['name'], model, description, price
        scraperwiki.sqlite.save(['timestamp', 'product', 'model'], {'timestamp' : scrape_datetime, 'product' : product['name'], 'model' : model, 'description' : description, 'price' : price, 'save' : save})from BeautifulSoup import BeautifulSoup
import scraperwiki
import time
import re
...
for product in products:
    ...
    for item in items:
        ...
        #print scrape_datetime, product['name'], model, description, price
        scraperwiki.sqlite.save(['timestamp', 'product', 'model'], {'timestamp' : scrape_datetime, 'product' : product['name'], 'model' : model, 'description' : description, 'price' : price, 'save' : save})