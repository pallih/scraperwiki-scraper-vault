...
if(!$maxlong) $maxlong = strtotime('2011-10-01');
    ...
    $maxlong = strtotime($maxlong['date']);
    $year = date('Y',$maxlong);
    if($year < date('Y')) $year++;
...
foreach($dates[1] as $key => $week) {
    ...
    // Deduce if this is new descriptionrmation
    if($maxlong < $f) {
...
print "strongevents" . $events;
                ...
                foreach($es as $event) {
                    ...
                    $description = explode('</strong>',$event);
print "sdescription";
print_r($description);
                    $data = date_time_place($date,$description[0]);
                    $data['description'] = trim(utf8_encode(strip_tags($description[1])));
$linkweek = $week; # - 1;
$link = "http://president.ie/index.php?section=6&engagement=".$linkweek."&lang=eng";
$data['link'] = $link;
$data['guid'] = rand();
...
$description = explode('<br />',$event);
print "descriptionpb";
print_r($description);     $data = date_time_place($date,$description[0]);
                    $data['description'] = utf8_encode(strip_tags($description[1]));
$linkweek = $week; # - 1;
$link = "http://president.ie/index.php?section=6&engagement=".$linkweek."&lang=eng";
$data['link'] = $link;
$data['guid'] = rand();
...
print "PeventsP" . $events;
                foreach($es as $event) {
print "descriptionp";                    $description = explode('</span>',$event);
print_r($description);                    $data = date_time_place($date,$description[0]);
                    $data['description'] = trim(utf8_encode(strip_tags($description[1])));
$linkweek = $week; # - 1;
$link = "http://president.ie/index.php?section=6&engagement=".$linkweek."&lang=eng";
...
print "PeventsP" . $events;
                foreach($es as $event) {
print "descriptionp";                    $description = explode('</p>',$event);
print_r($description);                    $data = date_time_place($date,$description[0]);
                    $data['description'] = trim(utf8_encode(strip_tags($description[1])));
$linkweek = $week; # - 1;
$link = "http://president.ie/index.php?section=6&engagement=".$linkweek."&lang=eng";
...
print_r($timeplace);
    ...
    if($unix_date > 0) {
        ...
        if(empty($apm)) {
            ...
            $data['title'] = utf8_encode(strip_tags($str));
        } else {
            $title = explode($apm,$str);
            $data['title'] = utf8_encode(strip_tags(trim($title[1])));
            if($data['title'][0] == ':') $data['title'] = substr($data['title'],1);
        }
...
function save_data($data) {
    if(isset($data['date'])) {
        if($data['title'] != '' OR $data['description'] != '') {
            ...
            echo "\t".$data['startdate'].': '.$data['title'].' / '.$data['title'].$data['link']."\n";
            return true;
        } else return false;
    } else return false;
}
