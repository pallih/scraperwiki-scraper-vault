...
#<tr>
#<td height="21">Clare</td>
#<td align="right">4</td>
#<td>Pat Breen TD, Joe Carey TD</td>
#</tr>
for url in urls:
    print "Scraping", url
    ...
    if page is not None:
        ...
        #name = re.findall("<title>(.*?)</title>", page, re.DOTALL)
        # re.DOTALL makes it work across multiple lines as well.
        billnumber = re.findall("<em>(.*?)</em>", page, re.DOTALL)
        sponsoredby = re.findall("<strong>Birth Place/Date:</strong>(.*?)<br/>", page, re.DOTALL) 
        ...
        data = {'url': url, 'billnumber': billnumber, 'sponsoredby':sponsoredby } #,, 'source':source, 'method':method, 'status':status, 'start':start, 'startb':start, 'enacted':enacted,  'enactedb':enactedb}
