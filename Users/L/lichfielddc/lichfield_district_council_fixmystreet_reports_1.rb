...
def search_for_new_reports(until_date=Date.today)
  ...
  url = "http://lichfielddc.fixmystreet.com/open311/v2/requests.xml?jurisdiction_id=fiksgatami.no&agency_responsible=2434%7C2240&start_date=#{(until_date - 14).strftime('%Y-%m-%d')}&end_date=#{until_date.strftime('%Y-%m-%d')}"
  ...
  rows.each do |row|
      ...
      details = {
        ...
        :url => "http://lichfielddc.fixmystreet.com/report/#{row['service_request_id']}",
