...
class DOBScraper
  ...
  def initialize(name_database)
    ...
    @population = prize_winners = ScraperWiki.select(           
      "name, url, field from #{name_database}.swdata 
       order by name"
    )
    @last_saved_name = ScraperWiki.get_var('last_saved_name') 
  end

  def dump_dob(name, field, dob, star_sign)
    data = {
      'name' => name,
      'field' => field,
      'dob' => dob,
  ...
  def extract_dob(person)
    name,url,field = person['name'], person['url'], person['field']
    begin
    ...
    doc.css('table.infobox th').each do |th|
      if th.inner_text == "Born"
        ...
        begin
          ...
          dump_dob(name, field, dob, star_sign)
        rescue StandardError => error
  ...
  def skip_person?(name)
    ...
    name_index = @population.find_index{|person| person['name'] == name}
    last_saved_index = @population.find_index{|person| person['name'] == @last_saved_name}
...
dob_scraper = DOBScraper.new('biologists_chemists_and_physicists_names_and_wiki_')
dob_scraper.scrape
