...
c = 0
for followers_list in group(followers_json, 30):
    c = c + 1
    if c < batchstart:
        continue
    print "number", c, "out of", len(followers_json) / 30
    print 'batch of ids:', followers_list
    url = 'http://api.twitter.com/1/users/lookup.json?user_id=%s' % (urllib2.quote(','.join(map(str, followers_list))))
    ...
    scraperwiki.sqlite.save_var('batchdone', c)import scraperwiki
import simplejson
import urllib2
import sys
