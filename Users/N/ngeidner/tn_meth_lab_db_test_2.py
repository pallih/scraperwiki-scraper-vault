...
def scrape_table(soup):
    ...
    data_table = soup.find("table", { "id" : "ctl00_ContentPlaceHolder1_dgAddress" })
    ...
    for row in rows:
        ...
        if table_cells: 
            record['Select'] = table_cells[0].text
            record['Number'] = table_cells[1].text
            record['StreetNum'] = table_cells[2].text
            record['Street'] = table_cells[3].text
            record['City'] = table_cells[4].text
            record['County'] = table_cells[5].text
            record['State'] = table_cells[6].text
            record['Zip'] = table_cells[7].text
            record['Date'] = table_cells[8].text
            # Print out the data we've gathered
            print record, '------------'
            # Save the record to the datastore - 'Name' is our unique key - 
            scraperwiki.sqlite.save(["MethLabs"], record)
...
starting_url = 'http://www.rid-meth.org/frmSearchSite2.aspx'
