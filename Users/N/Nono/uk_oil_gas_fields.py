...
def parseDetailPage(url):
    ...
    name = root.xpath("string(//td[@class='s6'])").strip()
    partMatcher = re.match(r"([A-Z. {}-]+[A-Z. }])+?\{p(?:ar)?t\.? ?of ([A-Z0-9 ]+)\}", name)
    if partMatcher:
        fieldInfo['name'] = partMatcher.group(1).strip().replace('{', '(').replace('}', ')')
        if partMatcher.group(2) is not None:
            fieldInfo['part_of'] = partMatcher.group(2).strip()
    else:
        fieldInfo['name'] = name.strip().replace('{', '(').replace('}', ')')
    ...
    for entry in ownership:
        ...
        if ownerInfo['owner'].find(fieldInfo['operator']) == 0:
            fieldInfo['operator'] = ownerInfo['owner']
...
def parseProdPage(url, product):
    ...
    partMatcher = re.match(r"([A-Z. {}-]+[A-Z. }])+?\{p(?:ar)?t\.? ?of ([A-Z0-9 ]+)\}", name)
    if partMatcher:
        name = partMatcher.group(1).strip().replace('{', '(').replace('}', ')')
    else:
    ...
    for prod in prods:
        ...
        prodInfo['date'] = prod.xpath("../td[position()=1]/text()")[0] + "/" + str((i % 12) + 1)
...
try:
    ...
    scraperwiki.sqlite.execute("create table if not exists field_info (name text, part_of text)")
