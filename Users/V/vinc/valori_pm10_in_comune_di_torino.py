    ...
    for t in tables:
        ...
        for n, td in enumerate(t.findAll('td')):
            if td.contents[0].string in months.keys():
                ...
                scraperwiki.sqlite.save(unique_keys=['pm10_value', 'year', 'month', 'day', 'location'], data={'pm10_value':pm10_value, 'year': year, 'month': month, 'day': n, 'location': location})

for l in locations:
    for y in years:
        extract_data(location=l,year=y)
        print "%s %s done" % (l, y)import scraperwiki
import mechanize 
from BeautifulSoup import BeautifulSoup
