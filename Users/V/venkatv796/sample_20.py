    ...
    if countrycode.strip() == "US":
        ...
        data_dict = {
                       ...
                       'relationship_properties':
                        ...
                        'company_properties':
                                       {
                                         'federal_authorized': federal_auth,
                                         'state_authorized': localAndEduAuth,
                                         'local_authorized': localAndEduAuth,
                                         'edu_authorized': localAndEduAuth
                                        }
                     }
...
for item in data_list:
    ...
    if details is not None:
        ...
        item['company_overview'] = item['revenue'] = item['employees'] = item['primary_phone'] = item['industries'] = item['address'] = None
    ...
    if address is not None:
        ...
        parsed_address = parse_google_address_components(data)
        state_code = parsed_address['state_code']
        pincode = parsed_address['pincode']
        formatted_address = parsed_address['address_formatted']
        if formatted_address in EMPTY_VALUES:
            formatted_address = address
        item['address'] = {'state': state_code,'pincode': pincode,'address_formatted': formatted_address}
    else:
        item['address'] = {'state': "",'pincode': "",'address_formatted': ""}
    data_list[index] = item
    scraperwiki.sqlite.save(unique_keys=[], data=item, table_name="napp_reseller", verbose=2)
    index += 1
...
for item_list in value_list:
    ...
    while counter < len(item_list):
        ...
        key = str(key_list[counter])
        tmp_dict[key] = value
        counter += 1
    #print "tmp dict:%s" %(repr(tmp_dict))    
...
for item in data_list_formatted:
    ...
    if address is not None:
        ...
        item['address'] = {'state': "",'pincode': "",'address_formatted': ""}
    scraperwiki.sqlite.save(unique_keys=[], data=item, table_name="napp_reseller_expanded", verbose=2)
