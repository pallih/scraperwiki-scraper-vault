...
def process_constituency(number, constituency):
    ...
    for x in results[3:-1]: #skip the first 3 tr's and the last one
        ...
        #print record
        scraperwiki.sqlite.save(unique_keys=['candidate', 'constituency'], data=record, table_name='nagaland-elections')   
    

#######################

#Scrape the first page to get the list of constituencies
html = scraperwiki.scrape('http://eciresults.ap.nic.in/Constituencywises171.htm')

#The list of constituencies for Himachal is in this markup:
# <input type="hidden" id="hdnFldNagaland" value=
