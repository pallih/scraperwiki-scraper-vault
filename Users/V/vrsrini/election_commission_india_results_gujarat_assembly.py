...
def process_constituency(number, constituency):
    ...
    for x in results[3:-1]: #skip the first 3 tr's and the last one
        ...
        scraperwiki.sqlite.save(unique_keys=['candidate', 'constituency'], data=record, table_name='gujarat-elections')   
    

#######################

#Scrape the first page to get the list of constituencies
html = scraperwiki.scrape('http://eciresults.ap.nic.in/ConstituencyWises061.htm')
